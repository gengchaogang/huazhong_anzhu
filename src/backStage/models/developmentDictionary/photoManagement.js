import {initialization,getCommunityUrls} from '../../services/developmentDictionary/photoManagement';
  ...
  subscriptions:{
		setup({ dispatch, history }) {
			history.listen(location => {
				if (location.pathname === '/developmentDictionary/residentialArea/photoManagement') {
	...
	effects: {
      ...
      const  {data,err}  = yield call(getCommunityUrls, parse(payload));
			if(err){
				return message.error('出错了，请联系管理员');
			}
      if(data.status=='success'){
