import {findAll,addRooms,editRooms} from '../../services/developmentDictionary/watchHousingNumberOffice';
import { parse } from 'qs';
import { message} from 'antd';
export default {
	namespace: 'watchHousingNumberOffice',
	state: {
    id:'',
    totalNumberOfFloors:'',
    household:'',
  ...
  subscriptions:{
		setup({ dispatch, history }) {
			history.listen(location => {
				if (location.pathname === '/developmentDictionary/officeBuilding/watchHousingNumberOffice') {
					// console.log(location.state);
					dispatch({
            type:'findAll',
            payload:{
							buildingId:location.state.id,
							totalNumberOfFloors:location.state.totalNumberOfFloors,
              household:location.state.household,
              roomPrefix:location.state.roomPrefix,
