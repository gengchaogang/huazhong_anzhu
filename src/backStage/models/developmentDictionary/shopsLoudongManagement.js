import {initialization,deleteShopBuilding,addShopBuilding,
findShopBuilding,editShopBuilding,findGroup} from '../../services/developmentDictionary/shopsLoudongManagement';
import { parse } from 'qs';
import { message} from 'antd';
export default {
	namespace: 'shopsLoudongManagement',
	state: {
		id:'',
		jianzhuType:[],//建筑类型
    dataSource:[],
  ...
  subscriptions:{
		setup({ dispatch, history }) {
			history.listen(location => {
				if (location.pathname === '/developmentDictionary/shopsManagement/shopsLoudongManagement') {
					dispatch({
						type:'initialSuccess',
						payload:{
							loading:true,
            ...
            type: 'initial',
						payload:{
							...
							shangPuName:location.state.shangPuName,
							pageNo:0,
						}
          });
				}
    ...
    *initial({ payload }, { call, put }){
      const  {data,err}  = yield call(initialization, parse(payload));
        ...
        yield put ({
          ...
          payload: {
						...
						shangPuName:payload.shangPuName,
						loading:false,
          },
        });
      }
      ...
      if(data.data){
		...
		*deleteOk({ payload }, { call, put,select }){
			const  {data,err}  = yield call(deleteShopBuilding, parse(payload));
			if(err){
				return message.error('出错了，请联系管理员');
			}
			if(data.status=='success'){
				const communityId=yield select(({shopsLoudongManagement})=>shopsLoudongManagement.communityId)
		...
		*handleOk({ payload }, { call, put}){
			const  {data,err}  = yield call(addShopBuilding, parse(payload));
			if(err){
				return message.error('出错了，请联系管理员');
			}
			if(data.status=='success'){
		...
		*editClick({ payload }, { call, put}){
			const  {data,err}  = yield call(findShopBuilding, parse(payload));
			if(err){
				return message.error('出错了，请联系管理员');
			}
			if(data.data){
		...
		*edithandleOk({ payload }, { call, put}){
			const  {data,err}  = yield call(editShopBuilding, parse(payload));
			if(err){
				return message.error('出错了，请联系管理员');
			}
			if(data.status=='success'){
