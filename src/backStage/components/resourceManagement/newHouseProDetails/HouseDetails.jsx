  ...
  switch (type) {
    ...
    return(
      <DxPanel title="房源信息">
        ...
        <Row className="row">
          ...
          {<Col span={8}>总层数：{detailsData.totalFloors+'层'}</Col>}
          <Col span={8}>期望/成交总价：{renderTotalMoneyStr(detailsData.totlePrice)}</Col>
        </Row>
        <Row className="row">
      ...
      <DxPanel title="房源信息">
        ...
        <Row className="row">
          ...
          <Col span={8}>可经营类别：
            ...
            {!!detailsData.businessCategory && catchJson(detailsData.businessCategory).map((item,index)=>(
              <Tag color="#87d068" key={`item_${index}`}>{item}</Tag>
            ))}
          </Col>
        </Row>
    ...
    return(
      <DxPanel title="房源信息">
          ...
          <Row className="row">
            <Col span={8}>使用面积：{detailsData.innerArea}㎡</Col>
            <Col span={8}>销售总价：{renderTotalMoneyStr(detailsData.totlePrice)}</Col>
              <Col span={8}>写字楼类型：
