...
function ClosedTable({location, dispatch, cityManagement,form}) {
	const { getFieldDecorator, setFields ,validateFields,resetFields,} =form;
	const {tableLoading, response}=cityManagement.tableClose;
	const {cityClosedCode,eidtCityModalClosed}=cityManagement;
		...
		if(dataIndex==='operation'){
			...
			let editClosed=()=>{
				dispatch({
					...
					payload:{
						eidtCityModalClosed:true,
						cityClosedCode:record.code
					}
				})
			}
			let deleteClosed=()=>{
			...
			return(
				<span>
					<a className='operationColor' onClick={openCity}>重新开通</a>
					<a className='operationColor' onClick={toDivisionManagement}>片区管理</a>
					<a className='operationColor' onClick={editClosed}>编辑</a>
					<a className='operationColor' onClick={deleteClosed}>删除</a>
				</span>
			);
		}
	};
	...
	const cityClosedOnOk=()=>{
		form.validateFields((err, values) => {
			if (err) {
				return;
			}
			dispatch({
				type:'cityManagement/eidtCityModalClosed',
				payload:{
					code:cityClosedCode,
					py:values.py,
				}
	...
	return (
		<div>
			<Modal title="编辑" visible={eidtCityModalClosed}
          onOk={cityClosedOnOk} onCancel={cityClosedOnOnCancel}
        >
				<FormItem
					label="城市拼音"
					{...formItemLayout}
...
ClosedTable.propTypes = {

}
ClosedTable = Form.create({})(ClosedTable);
function mapStateToProps({ cityManagement }) {
	return { cityManagement }
}

export default connect(mapStateToProps)(ClosedTable)
