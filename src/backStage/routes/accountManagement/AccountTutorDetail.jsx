...
import './AccountTutorDetail.css';
import DxPanel from '../../../commons/components/DxPanel'
const TabPane = Tabs.TabPane;
const RadioButton = Radio.Button;
const RadioGroup = Radio.Group
const { MonthPicker, RangePicker } = DatePicker;
function AccountTutorDetail({dispatch,accountTutorDetail,form}) {
	const {
    amount,
    avatar,
    bank,
	...
	const walletDetail=(key)=>{
		dispatch(routerRedux.push({
			pathname:'/accountManagement/accountTutor/tutorDetailed',
			state:{
				id:key.key,
				type:key.type,
			}
	...
	const callback=(key)=>{
		dispatch({
			type:'accountTutorDetail/initialSuccess',
			payload:{loading:true}
		})
		if(key=='钱包'){
			dispatch({
				type:'accountTutorDetail/findTutorWalletBudget',
				payload:{
					beginTime:'',
					endTime:'',
					pageNo:0,
			...
			dispatch({
				type:'accountTutorDetail/findTutorCommissionBudget',
				payload:{
					beginTime:'',
					endTime:'',
					pageNo:0,
	...
	const commissionColumns=[
		...
		{
			title:'操作',
			render:(text,record)=><span onClick={()=>{commissionDetail(record.key)}} className='zhangHuDetail'>明细</span>
		},
	]
	//分页
  const pagination={
          ...
          <Tabs onChange={callback} type="card">
            ...
            <TabPane tab="佣金记录" key="佣金记录">
							<div className='balanceMark'>
								{/*<span>未发放佣金：</span>
								<span>已发放佣金：</span>*/}
								<span>累计获得佣金：{totalCommission||0}元</span>
							</div>
							<div className='rangePicker'>
								<RangePicker onChange={onChangeWallet} />
							</div>
							{/*<div className='classification'>
								<span>状态：</span>
								<RadioGroup defaultValue="全部" onChange={typeWalletChange} value={status==''?'全部':status}>
									<RadioButton value="全部">全部</RadioButton>
					        <RadioButton value="提前到账">提前到账</RadioButton>
					        <RadioButton value="结算中">结算中</RadioButton>
					        <RadioButton value="已发放">已发放</RadioButton>
					        <RadioButton value="已入账">已入账</RadioButton>
					        <RadioButton value="失败">失败</RadioButton>
								</RadioGroup>
							</div>*/}
...
AccountTutorDetail.propTypes = {

};
function mapStateToProps({accountTutorDetail }) {
	return { accountTutorDetail }
}

export default connect(mapStateToProps)(Form.create()(AccountTutorDetail))
