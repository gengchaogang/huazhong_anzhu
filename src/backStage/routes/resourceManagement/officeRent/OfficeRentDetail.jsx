...
function OfficeRentDetail({dispatch,officeRentDetail,form}){
  ...
  const steps = [{
    title: '房源详情',
    content: <OfficeRentDetailOne listOwer={listOwer} houseBaseInfo={houseBaseInfo} onClick={()=>stepwatch(0)}/>,
    },
    {
      title: '视频图片',
      content: <OfficeRentDetailImg houseBaseInfo={houseBaseInfo} onClick={()=>stepwatch(1)}/>,
    },
    {
    title: '代理经纪人',
    content: <OfficeRentDetailBroker brokerPromotion={brokerPromotion} onClick={()=>stepwatch(2)}/>,
    },
    {
    title: '带看记录',
    content: <OfficeRentDetailWatchDetail guideArr={guideArr} total={total} pageNo={pageNo} pageChange={pageChange} radioChange={radioChange}  onClick={()=>stepwatch(3)}/>,
    },
    {
    title: '成交',
    content: <OfficeRentDetailDone trackJSON={trackJSON} transCode={transCode} onClick={()=>stepwatch(4)}/>,
    },
  ];
    ...
    <div>
      <DxPanel title=" ">
        <Steps current={current}>
          {steps.map(item => <Step key={item.title} style={{cursor:'pointer'}} title={item.title} onClick={()=>stepwatch(item.title)}/>)}
        </Steps>
      </DxPanel>
