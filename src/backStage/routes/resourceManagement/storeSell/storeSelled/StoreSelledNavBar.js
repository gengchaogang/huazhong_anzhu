...
function StoreSelledNavBar(props){
  const {dispatch,storeSelledNavBar,children}=props;
  const {current}=storeSelledNavBar
  const steps =[
    '房源详情',
    '视频图片',
    '代理经纪人',
...
function mapStateToProps({ storeSelledNavBar}) {
  return { storeSelledNavBar }
}

export default connect(mapStateToProps)(StoreSelledNavBar)
