...
function SecHandSelledNavBar(props){
  const {dispatch,secHandSelledNavBar,children}=props;
  const {current}=secHandSelledNavBar
  const steps =[
    '房源详情',
    '视频图片',
    '代理经纪人',
...
function mapStateToProps({ secHandSelledNavBar}) {
  return { secHandSelledNavBar }
}

export default connect(mapStateToProps)(SecHandSelledNavBar)
