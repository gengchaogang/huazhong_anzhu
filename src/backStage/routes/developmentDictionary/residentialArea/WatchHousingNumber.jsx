...
function WatchHousingNumber({dispatch,watchHousingNumber,form}) {
  const {household,id,totalNumberOfFloors,roomNumberStartingValue,
    setHouseNoStatus,dataSurce,orgin,
    choseRulesStatus,loading,isCreate
  }=watchHousingNumber;
  const {
    getFieldDecorator,
    validateFields,
          ...
          return (
            <FormItem>
              {getFieldDecorator(`${record.key}_room${i}`, {
                ...
                <Input placeholder='请输入起始房号，如101'/>
              )}
            </FormItem>
          )
        }
  ...
  const setHouseNumber=()=>{
    dispatch({
      type:'watchHousingNumber/querySuccess',
      payload:{
  ...
  const setHouseNumberOk=()=>{
    form.validateFields((err, values) => {
      ...
      dispatch({type:'watchHousingNumber/querySuccess',
        payload:{loading:true}
      })
      dispatch({
        type:'watchHousingNumber/addHouseNumber',
        payload:{
          rooms:arr,
          buildingId:id,
        }
  ...
  const editClick=()=>{
    form.validateFields((err, values) => {
      ...
      dispatch({
        type:'watchHousingNumber/editRoomsNumber',
        payload:{
          rooms:arr,
          buildingId:id,
        }
...
function mapStateToProps({watchHousingNumber}) {
  return {watchHousingNumber}
}

export default connect(mapStateToProps)(Form.create()(WatchHousingNumber));
