...
import './WatchHousingNumberShop.css'
...
function WatchHousingNumberShop({dispatch,watchHousingNumberShop,form}) {
  const {household,id,totalNumberOfFloors,roomNumberStartingValue,roomPrefix,
    setHouseNoStatus,dataSurce,orgin,loading,choseRulesStatus,isCreate}=watchHousingNumberShop;
  const {
    getFieldDecorator,
    validateFields,
    getFieldsValue,
          ...
          return (
            <FormItem>
              {getFieldDecorator(`${record.key}_room${i}`, {
                ...
                <Input addonBefore={roomPrefix} placeholder='请输入起始房号，如101'/>
              )}
            </FormItem>
          )
        }
  ...
  const setHouseNumber=()=>{
    dispatch({
      type:'watchHousingNumberShop/querySuccess',
      payload:{
  ...
  const setHouseNumberOk=()=>{
    form.validateFields((err, values) => {
      ...
      dispatch({
        type:'watchHousingNumberShop/addHouseNumber',
        payload:{
          rooms:arr,
          buildingId:id,
        }
  ...
  const editClick=()=>{
    form.validateFields((err, values) => {
      ...
      dispatch({
        type:'watchHousingNumberShop/editRoomsNumber',
        payload:{
          rooms:arr,
          buildingId:id,
        }
  ...
  const clearGeneration=()=>{
    form.validateFields((err, values) => {
      ...
      dispatch({type:'watchHousingNumberShop/querySuccess',
  ...
  return (
    <DxPanel title='房源管理'>
      ...
      <Modal title='添加楼层房号' visible={setHouseNoStatus}
        onOk={setHouseNumberOk} onCancel={choseRulesCancel}
      >
        <Form>
          <FormItem
            label='设置房源起始值'
...
function mapStateToProps({watchHousingNumberShop}) {
  return {watchHousingNumberShop}
}

export default connect(mapStateToProps)(Form.create()(WatchHousingNumberShop));
