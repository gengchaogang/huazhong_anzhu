...
function ShopsImgsRent({ dispatch, shopsImgsRent }) {
  const {
    showPicList,
    promptObj,
    loadingShadow,
  ...
  var { houseBaseInfo } = shopsImgsRent;
  ...
  const upLoadPicProps = {
    ...
    changeList: (data) => {
      dispatch({
        type: "shopsImgsRent/deletePic",
        payload: data
      })
      dispatch({
        type: "shopsImgsRent/showPicList",
        payload: data
      })
    },//更新list回调
  }
  ...
  const onOkCallBack = () => {
    if (promptObj.todo === 'closeModal') {
      dispatch({
        type: "shopsImgsRent/togglePrompt",
        payload: {
          visible: false
        }
      })
...
function mapStateToProps({ shopsImgsRent }) {
  return { shopsImgsRent }
}
export default connect(mapStateToProps)(ShopsImgsRent)
