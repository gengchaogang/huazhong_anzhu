...
function ShopsResourceInfosRent({dispatch,form,shopsResourceInfosRent}){
    ...
    form.validateFields((err, fieldsValue) => {
      if (err) {
        ...
        if(storey > totalFloors){
          ...
          //整理数据发起请求
          dispatch({
            type:'shopsResourceInfosRent/submitData',
            payload:fieldsValue,
          });
      }
    })
  ...
  const onSelectChange=(value, selectedOptions)=>{
    ...
    // if(value.length){
      dispatch({
        type:"shopsResourceInfosRent/changeDisabled",
        payload:{
      ...
      dispatch({
        type:'shopsResourceInfosRent/setState',
        payload:{
            areaName:areaName,
      ...
      dispatch({
        type:"shopsResourceInfosRent/searchCommunities",
        payload:{
          areaName:areaName,
          type:"商铺"
        }
  ...
  const tagsChange=(checked)=>{
    dispatch({
      type:"shopsResourceInfosRent/changeTagsValue",
      payload:{
        tagsValue:checked
      }
    })
  ...
  const onOkCallBack=()=>{
      if(promptObj.todo==='closeModal'){
        dispatch({
          type:"shopsResourceInfosRent/togglePrompt",
          payload:{
            visible:false
          }
        })
  ...
  const handleSelectBlur=(value)=>{
    ...
    if(communityData.length){
      communityData.map((item,index)=>{
        if(item.name===value){
          dispatch({
            type:"shopsResourceInfosRent/getBuildingsData",
            payload:{
              id:item.id
            }
          })
  ...
  const handleBuildingNameSelect=(value, option)=>{
    dispatch({
      type:"shopsResourceInfosRent/getRoomsData",
      payload:{
        id:value
      }
    })
  ...
  const checkHouseUnique=(value)=>{
    ...
    dispatch({
      type:"shopsResourceInfosRent/saveHouseUnique",
      payload:{
    ...
    dispatch({
      type:"shopsResourceInfosRent/checkHouseUnique",
      payload:{
        buildingId:buildingId,
        communityId:communityId,
        houseNumberId:houseNumberId,
                ...
                <FormItem label="楼盘名称"
                  ...
                  >
                  {
                    form.getFieldDecorator('communityName',{
                      ...
                      rules:[{required:true,message:'请选择或输入商铺名称!'}, { max: 30, message: "长度过长" }],})(
                        <Select
                        ...
                        {getFieldDecorator('uintPirce', {
                            initialValue: houseBaseInfo.uintPirce,
                             ...
                             <Select placeholder="请选择" onChange={handleSelectChange}>
                                {
                                 getSelectOptionsByLabelName(labelsFinalCode.labelShops_czzjdw)
                                }
                             </Select>
                            )}
                        </FormItem>
              ...
              <Col sm={24} md={24}>
                ...
                {/* <FormItem label="佣金" {...formItemLayout}>
                  {getFieldDecorator('maxCommission', {
                    ...
                    <Input placeholder="请输入佣金" addonAfter="元"/>
                  )}
                </FormItem> */}
              </Col>
            </Row>
                ...
                <FormItem {...formItemLayoutLabel}>
                  {getFieldDecorator('characteristics', {
                    ...
                    rules: [
                      ...
                      <CheckableTags
                        multiple={true}
                        tags={labelsFinalCode.getlabelsByTypeName(labels,labelsFinalCode.labelShopsCzts)}
                        customStyle={customStyle}
                        onChange={tagsChange}
...
function mapStateToProps({shopsResourceInfosRent}){
  return{shopsResourceInfosRent}
}

export default connect(mapStateToProps)(Form.create({})(ShopsResourceInfosRent));
