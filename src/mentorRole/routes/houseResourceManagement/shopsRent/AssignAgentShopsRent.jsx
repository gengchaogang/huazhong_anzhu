...
import './assignAgentShopsRent.css'
...
function AssignAgentShopsRent({dispatch,assignAgentShopsRent,form}){
  ...
  const onExpand = (expandedKeys) => {
      dispatch({
        type:"assignAgentShopsRent/setStatePramas",
        payload:{
          expandedKeys,
    ...
    const checkedTreeNode=(checkedKeys,e)=>{  //勾选
      ...
      dispatch({
        type:"assignAgentShopsRent/saveAgentTableData",
        payload:{
  ...
  const onDelete=(text,record,index)=>{
    if (slectedBrokers != null){
        ...
        dispatch({
          type:"assignAgentShopsRent/setState",
          payload:{
  ...
  const showModal=()=>{
    dispatch({
      type:"assignAgentShopsRent/currentSelect",
      payload:{
        agentListTableData:agentListTableData,
    ...
    // dispatch({
    //   type:"assignAgentShopsRent/changeModal",
    //   payload:{
    //     visible:true
    //   }
    // });
    // dispatch({
    //   type:'assignAgentShopsRent/getBrokerList',
    //   payload:{
    //     pageNo:0,
  ...
  const onModalOk=(data)=>{
    ...
    // 保存选择的经纪人信息
    dispatch({
        type:"assignAgentShopsRent/addSelectBroker",
        payload:data,
    });
  }
  // 选择经纪人取消
  ...
  const onOkCallBack=()=>{
      if(promptObj.todo==='closeModal'){
        dispatch({
          type:"assignAgentShopsRent/togglePrompt",
          payload:{
            visible:false
          }
        })
          ...
          setTimeout(function(){
            dispatch({
              type:"assignAgentShopsRent/backlist",
            })
          }, 200)
      }
  }
  ...
  const toBack=()=>{
    dispatch(routerRedux.push({
      pathname:'/houseResourceRentManagement/shopsRent/createShopsRentResource/shopsOwnerInfosRent',
    ...
    form.validateFields((err, fieldsValue) => {
     ...
     if (err) {
       ...
       if(!!fieldsValue.commissionRate&&fieldsValue.commissionRate!==''){
        ...
        dispatch({
          type:"assignAgentShopsRent/submitData",
          payload:fieldsValue,
        });
     }
   });
  ...
  const openPromotionCheckBox=(e)=>{
      ...
      if (isChecked) {
        ...
        // 关闭合作促销
        dispatch({
          type:"assignAgentShopsRent/clearCxInfo",
        });
        form.setFieldsValue({
  ...
  const onInputChange=(e)=>{
    ...
    dispatch({
      type:"assignAgentShopsRent/closeCommissionAmount",
      payload:{
  ...
  const onRadioGroupChange=(e)=>{
    ...
    dispatch({
      type:"assignAgentShopsRent/saveCurrentRadio",
      payload:{
                ...
                <Col sm={24} md={24}>
                  <FormItem label="房源销售总佣金" {...formItemLayout}>
                    {getFieldDecorator('tradingCommissions', {
                      initialValue: !!assignAgentShopsRentObj.tradingCommissions?assignAgentShopsRentObj.tradingCommissions.toString():null,
                      rules: [
                              ...
                              <Radio style={radioStyle} value='佣金金额'>佣金金额
                                <FormItem className="commissionType">
                                    {getFieldDecorator('commissionAmount', {
                                      initialValue: assignAgentShopsRentObj.promotionMode==='金额'?assignAgentShopsRentObj.commissionAmount.toString():null,
                                      rules: [
                              ...
                              <Radio style={radioStyle} value='佣金比例'>佣金比例
                                    <FormItem className="commissionType">
                                        {getFieldDecorator('commissionRate', {
                                          initialValue: !!assignAgentShopsRentObj.commissionRate?(assignAgentShopsRentObj.commissionRate*100).toString():null,
                                          rules: [
...
function mapStateToProps({assignAgentShopsRent}){
  return{assignAgentShopsRent}
}
export default connect(mapStateToProps)(Form.create({})(AssignAgentShopsRent));
