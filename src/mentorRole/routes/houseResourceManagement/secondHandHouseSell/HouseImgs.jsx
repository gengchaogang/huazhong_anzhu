...
function HouseImgs({ dispatch, houseImgs }) {
  const {
    showPicList,
    promptObj,
    loadingShadow,
  ...
  const toBack = () => {
    ...
    if (resourcesType == commonFinalCode.senondHouseResourcesType_zz) {
      ...
      if (saleWay == commonFinalCode.senondHouseSaleWay_cs) {
        ...
        // 出租
        dispatch(routerRedux.push({
          pathname: '/houseResourceSaleManagement/secondHandHouseRent/createSecondHandRentResource/houseResourceInfosRent',
          state: { //这里传入projectId 跳转到下一个页面时监听路由把值存在state中
            houseBaseInfo: houseBaseInfo
          }
        }))
      ...
      if (saleWay == commonFinalCode.senondHouseSaleWay_cs) {
        ...
        // 出租
        dispatch(routerRedux.push({
          pathname: '/houseResourceSaleManagement/shopsRent/createShopsRentResource/shopsResourceInfosRent',
          state: { //这里传入projectId 跳转到下一个页面时监听路由把值存在state中
            houseBaseInfo: houseBaseInfo
          }
        }))
      ...
      if (saleWay == commonFinalCode.senondHouseSaleWay_cs) {
        ...
        // 出租
        dispatch(routerRedux.push({
          pathname: '/houseResourceSaleManagement/officeRent/createOfficeRentResource/officeResourceInfosRent',
          state: { //这里传入projectId 跳转到下一个页面时监听路由把值存在state中
            houseBaseInfo: houseBaseInfo
          }
        }))
...
function mapStateToProps({ houseImgs }) {
  return { houseImgs }
}
export default connect(mapStateToProps)(HouseImgs)
