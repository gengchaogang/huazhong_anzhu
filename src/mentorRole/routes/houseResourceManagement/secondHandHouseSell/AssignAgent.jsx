...
function AssignAgent({dispatch,assignAgent,form}){
  ...
  const {
    ...
    console.log("assignAgentObj",assignAgentObj)
  const onExpand = (expandedKeys) => {
      dispatch({
        type:"assignAgent/setStatePramas",
        payload:{
          expandedKeys,
    ...
    const checkedTreeNode=(checkedKeys,e)=>{  //勾选
      ...
      dispatch({
        type:"assignAgent/saveAgentTableData",
        payload:{
  ...
  const columns = [
    {
      ...
      render:(text,record,index)=>{
        ...
        if (_logo != null && _logo.length > 0) {
            ...
            return (
              <img src={img} className="listUserHeaderImage"/>
            );
        }
      },
      key:"logo"
  ...
  const showModal=()=>{
    dispatch({
      type:"assignAgent/currentSelect",
      payload:{
        agentListTableData:agentListTableData,
  ...
  const onModalOk=(data)=>{
    ...
    // 保存选择的经纪人信息
    dispatch({
        type:"assignAgent/addSelectBroker",
        payload:data,
    });
  }
  // 选择经纪人取消
  ...
  const toBack=()=>{
    dispatch(routerRedux.push({
      pathname:'/houseResourceSaleManagement/secondHandHouseSell/createSecondHandSellResource/ownerInfos',
  ...
  const onInputChange=(e)=>{
    ...
    dispatch({
      type:"assignAgent/closeCommissionAmount",
      payload:{
  ...
  const onRadioGroupChange=(e)=>{
    ...
    dispatch({
      type:"assignAgent/saveCurrentRadio",
      payload:{
                ...
                <Col sm={24} md={24}>
                  <FormItem label="房源销售总佣金" {...formItemLayout}>
                    {getFieldDecorator('tradingCommissions', {
                      initialValue: !!assignAgentObj.tradingCommissions?assignAgentObj.tradingCommissions.toString():null,
                      rules: [
                        { required: true, message:'请输入房源销售总佣金!' },
                        {type:'string',pattern:/^([1-9]\d*|)(\.\d{1,2})?$/,message:"房源销售总佣金，支持2位小数"},
                      ],
                    })(
                      <Input placeholder="请输入销售总佣金，支持2位小数" addonAfter='元'/>
                    )}
                  </FormItem>
                </Col>
                <Col sm={24} md={24}>
                              ...
                              <Radio style={radioStyle} value='佣金金额'>佣金金额
                                <FormItem className="commissionType">
                                    {getFieldDecorator('commissionAmount', {
                                      initialValue: assignAgentObj.promotionMode==='金额'?assignAgentObj.commissionAmount.toString():null,
                                      rules: [
                              ...
                              <Radio style={radioStyle} value='佣金比例'>佣金比例
                                    <FormItem className="commissionType">
                                        {getFieldDecorator('commissionRate', {
                                          initialValue: !!assignAgentObj.commissionRate?(assignAgentObj.commissionRate*100).toString():null,
                                          rules: [
                                            { required: false, message:'请选择佣金比例!' },
                                            {type:'string',pattern:/^(?:|[1-9][0-9]?|100)$/,message:"佣金比例范围只能是1-100的正整数"}
                                          ],
                                        })(
                                          <Input
                                            placeholder="请输入佣金比例"
                                          ...
                                          //   >
                                          //   {
                                          //     getSelectOptionsByLabelName(labels, labelsFinalCode.labelHzsx)
                                          //   }
                                          // </Select>
                                        )}
                                    </FormItem>
...
function mapStateToProps({assignAgent}){
  return{assignAgent}
}
export default connect(mapStateToProps)(Form.create({})(AssignAgent));
