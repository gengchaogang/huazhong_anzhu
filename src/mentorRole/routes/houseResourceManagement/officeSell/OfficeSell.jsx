...
/***************************************************************/
/**************************************************************/
/**********  写字楼出售：已发布，未指派经纪人、已下架、已售 *******/
/*************************************************************/
/*************************************************************/

function OfficeSell({ form, dispatch, mentorOfficeSell }) {
  const {
    pwd,
    tudiGongVisible,
    area,
  ...
  const paginationPublishedHouse = {
    ...
    onChange: (page, pageSize) => {
      dispatch({
        type: 'mentorOfficeSell/getNewHouseDealTableData',
        payload: {
          pageNo: page - 1,
          pageSize: publishedHousePage.pageSize,
          houseState: "已发布",
          ...
          saleWay: "出售",
          hasBroker: "是",
          keyword: mentorOfficeSell.resourcesNumber,
          fullPath: mentorOfficeSell.area,
        }
      })
    },
  };
  ...
  const paginationPause = {
    ...
    onChange: (page, pageSize) => {
      dispatch({
        type: 'mentorOfficeSell/getPauseList',
        payload: {
          pageNo: page - 1,
          pageSize: pausePage.pageSize,
          //  hasBroker:"否",
          ...
          saleWay: "出售",
          keyword: mentorOfficeSell.resourcesNumber,
          houseRoom: mentorOfficeSell.houseRoom,
          fullPath: mentorOfficeSell.area,
        }
      })
    },
  ...
  const paginationCooperation = {
    ...
    onChange: (page, pageSize) => {
      dispatch({
        type: 'mentorOfficeSell/getCooperationList',
        payload: {
          pageNo: page - 1,
          pageSize: cooperationPage.pageSize,
  ...
  const handleEdit = (text, record, index) => {
    dispatch(routerRedux.push({
      pathname: '/houseResourceSaleManagement/officeSell/createOfficeSellResource',
      state: {
        isUpdate: true,
        dataId: record['id'],
      }
  ...
  const handleEntrust = (text, record, index) => {//委托
    dispatch({
      type: "mentorOfficeSell/setState",
      payload: {
        entrustModal: {
          visible: true,
          required: true
  ...
  const handleDelete = (text, record, index) => {
    confirm({
      ...
      onOk() {
        dispatch({
          type: "mentorOfficeSell/deleteHouse",
          payload: record,
        })
      },
      onCancel() { },
  ...
  const onSelectChange = (value, selectedOptions) => {
    ...
    dispatch({
      type: "mentorOfficeSell/saveSearchArea",
      payload: {
        area: areaName
      }
    })
  ...
  const onTabsChange = (key) => {
    form.resetFields();
    dispatch({
      type: "mentorOfficeSell/saveSelectedRowKeys",
      payload: {
        selectedRowKeys: []
      }
    })
    ...
    dispatch({
      type: 'mentorOfficeSell/loadList',
      payload: {
        isReFresh: true
      }
    })
  ...
  const handleSubmit = (e) => {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        values.area = area;
        mentorOfficeSell.area = values.area;
        mentorOfficeSell.resourcesNumber = values.resourcesNumber;
        dispatch({
          type: "mentorOfficeSell/loadList",
          payload: {
            area: values.area,
  ...
  const onOkCallBack = () => {
    if (promptObj.todo === 'closeModal') {
      dispatch({
        type: "mentorOfficeSell/togglePrompt",
        payload: {
          visible: false
        }
      })
    ...
    if (promptObj.todo === 'closeModalAndWritePass') {
      ...
      dispatch({
        type: "mentorOfficeSell/changeVisible",
        payload: {
  ...
  const shareHouse = () => {
    dispatch({
      type: "mentorOfficeSell/sharHouse",
      payload: {
  ...
  const saveTudigongPass = (e) => {
    dispatch({
      type: "mentorOfficeSell/saveTudigongPass",
      payload: {
        pwd: e.target.value
      }
    })
  ...
  const houseStateSubmit = () => {
    form.validateFields((err, values) => {
      if (!err) {
        dispatch({
          type: "mentorOfficeSell/houseStateSet",
          payload: {
...
function mapStateToProps({ mentorOfficeSell }) {
  return { mentorOfficeSell }
}
export default connect(mapStateToProps)(Form.create({})(OfficeSell));
