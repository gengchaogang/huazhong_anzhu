...
function OfficeResourceInfos({dispatch,form,officeResourceInfos}){
      ...
      form.validateFields((err, fieldsValue) => {
        if (err) {
          ...
          return;
            ...
            //整理数据发起请求
            dispatch({
              type:'officeResourceInfos/submitData',
              payload:fieldsValue,
            });
        }
      })
  ...
  const onSelectChange=(value, selectedOptions)=>{
    ...
    //if(value.length){
      dispatch({
        type:"officeResourceInfos/changeDisabled",
        payload:{
      ...
      dispatch({
        type:'officeResourceInfos/setState',
        payload:{
            areaName:areaName,
      ...
      dispatch({
        type:"officeResourceInfos/searchCommunities",
        payload:{
          areaName:areaName,
          type:"写字楼"
        }
  ...
  const checkHouseUnique=(value)=>{
    ...
    dispatch({
      type:"officeResourceInfos/saveHouseUnique",
      payload:{
    ...
    dispatch({
      type:"officeResourceInfos/checkHouseUnique",
      payload:{
        buildingId:buildingId,
        communityId:communityId,
        houseNumberId:houseNumberId,
  ...
  const tagsChange=(checked)=>{
    dispatch({
      type:"officeResourceInfos/changeTagsValue",
      payload:{
        tagsValue:checked
      }
    })
  ...
  const onOkCallBack=()=>{
      if(promptObj.todo==='closeModal'){
        dispatch({
          type:"officeResourceInfos/togglePrompt",
          payload:{
            visible:false
          }
        })
  ...
  const handleSelectBlur=(value)=>{
    ...
    if(communityData.length){
      communityData.map((item,index)=>{
        if(item.name===value){
          dispatch({
            type:"officeResourceInfos/getBuildingsData",
            payload:{
              id:item.id
            }
          })
  ...
  const handleBuildingNameSelect=(value, option)=>{
    dispatch({
      type:"officeResourceInfos/getRoomsData",
      payload:{
        id:value
      }
    })
          ...
          <div className="houseInfos">
                ...
                <FormItem label="写字楼名称"
                  ...
                  >
                  {
                    form.getFieldDecorator('communityName',{
                      ...
                      rules:[{required: true, message:'请选择或输入写字楼名称!'}, { max: 30, message: "长度过长" }],})(
                        <Select
            ...
            <Row>
              <Col sm={24} md={24}>
                ...
                extra={isCommunitied?<div className="houseDetailAddress">(请先选择或输入写字楼!)</div>:null}
                  >
                ...
                <FormItem label="写字楼级别"
                  ...
                  >
                  {
                    form.getFieldDecorator('shopOfficeType',{
                      ...
                      rules:[{required: true, message:'请选择写字楼级别!'}],})(
                        <CheckableTags
                          multiple={false}
                ...
                <FormItem label="销售总价"
                  ...
                  >
                  {
                    form.getFieldDecorator('totlePrice',{
                      ...
                      rules:[
                        {required: true,message:'请输入销售总价!'},
                        {type:'string',pattern:/^([1-9]\d*|0)(\.\d{1,2})?$/,message:"销售总价，支持2位小数"},
                      ],})(
                        <Input placeholder="请输入销售总价" addonAfter="万元"  maxLength="15"/>
                    )
                  }
                </FormItem>
              </Col>
                ...
                <FormItem {...formItemLayoutLabel}>
                  {getFieldDecorator('characteristics', {
                    ...
                    rules: [
                      ...
                      <CheckableTags
                        multiple={true}
                        tags={labelsFinalCode.getlabelsByTypeName(labels, labelsFinalCode.labelXzlcsts)}
                        customStyle={customStyle}
                        onChange={tagsChange}
...
function mapStateToProps({officeResourceInfos}){
  return{officeResourceInfos}
}

export default connect(mapStateToProps)(Form.create({})(OfficeResourceInfos));
