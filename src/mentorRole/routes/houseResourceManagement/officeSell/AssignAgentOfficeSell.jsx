...
import './assignAgentOfficeSell.css'
...
function AssignAgentOfficeSell({dispatch,assignAgentOfficeSell,form}){
  ...
  const {
    ...
    defaultRadioValue,
    agentList,
    } = assignAgentOfficeSell;
  const onExpand = (expandedKeys) => {
      dispatch({
        type:"assignAgentOfficeSell/setStatePramas",
        payload:{
          expandedKeys,
    ...
    const checkedTreeNode=(checkedKeys,e)=>{  //勾选
      ...
      dispatch({
        type:"assignAgentOfficeSell/saveAgentTableData",
        payload:{
  ...
  const onDelete=(text,record,index)=>{
    if (slectedBrokers != null){
        ...
        dispatch({
          type:"assignAgentOfficeSell/setState",
          payload:{
  ...
  const showModal=()=>{
    dispatch({
      type:"assignAgentOfficeSell/currentSelect",
      payload:{
        agentListTableData:agentListTableData,
    ...
    // dispatch({
    //   type:"assignAgentOfficeSell/changeModal",
    //   payload:{
    //     visible:true
    //   }
    // });
    // dispatch({
    //   type:'assignAgentOfficeSellModal/getBrokerList',
    //   payload:{
    //     pageNo:0,
  ...
  const onModalOk=(data)=>{
    ...
    // 保存选择的经纪人信息
    dispatch({
        type:"assignAgentOfficeSell/addSelectBroker",
        payload:data,
    });
  }
  // 选择经纪人取消
  ...
  const onOkCallBack=()=>{
      if(promptObj.todo==='closeModal'){
        dispatch({
          type:"assignAgentOfficeSell/togglePrompt",
          payload:{
            visible:false
          }
        })
          ...
          setTimeout(function(){
            dispatch({
              type:"assignAgentOfficeSell/backlist",
            })
          }, 200)
      }
  }
  ...
  const toBack=()=>{
    dispatch(routerRedux.push({
      pathname:'/houseResourceSaleManagement/officeSell/createOfficeSellResource/officeOwnerInfos',
    ...
    form.validateFields((err, fieldsValue) => {
     ...
     if (err) {
       ...
       if(!!fieldsValue.commissionRate&&fieldsValue.commissionRate!==''){
        ...
        dispatch({
          type:"assignAgentOfficeSell/submitData",
          payload:fieldsValue,
        });
     }
   });
  ...
  const openPromotionCheckBox=(e)=>{
      ...
      if (isChecked) {
        ...
        // 关闭合作促销
        dispatch({
          type:"assignAgentOfficeSell/clearCxInfo",
        });
        form.setFieldsValue({
  ...
  const onInputChange=(e)=>{
    ...
    dispatch({
      type:"assignAgentOfficeSell/closeCommissionAmount",
      payload:{
  ...
  const onRadioGroupChange=(e)=>{
    ...
    dispatch({
      type:"assignAgentOfficeSell/saveCurrentRadio",
      payload:{
        currentRadio:e.target.value
      }
    })
  }
  console.log('assignAgentOfficeSellObj',assignAgentOfficeSellObj);
  //------------------- 选择框事件  end----------
  return(
        ...
        <div className="cooperativePromotion">
          <Form>
            <Row className="firstRow">
                <Col sm={24} md={24}>
                  <FormItem label="房源销售总佣金" {...formItemLayout}>
                    {getFieldDecorator('tradingCommissions', {
                      initialValue: !!assignAgentOfficeSellObj.tradingCommissions?assignAgentOfficeSellObj.tradingCommissions.toString():null,
                      rules: [
            ...
            {openPromotionCheked?
                <Row>
                          <Col sm={24} md={24}>
                              <FormItem label="佣金金额设置" {...formItemLayout}>
                              </FormItem>
                          </Col>
                          <Col offset={3}>
                              ...
                              <Radio style={radioStyle} value='佣金金额'>佣金金额
                                <FormItem className="commissionType">
                                    {getFieldDecorator('commissionAmount', {
                                      initialValue: assignAgentOfficeSellObj.promotionMode==='金额'?assignAgentOfficeSellObj.commissionAmount.toString():null,
                                      rules: [
                                        { required: false, message:'请输入佣佣金金额"!' },
                              ...
                              <Radio style={radioStyle} value='佣金比例'>佣金比例
                                    <FormItem className="commissionType">
                                        {getFieldDecorator('commissionRate', {
                                          initialValue: !!assignAgentOfficeSellObj.commissionRate?(assignAgentOfficeSellObj.commissionRate*100).toString():null,
                                          rules: [{ required: false, message:'请选择佣金比例!' }],
                                        })(
                                          <Input
                                            placeholder="请输入佣金比例"
                                            addonAfter="%"
...
function mapStateToProps({assignAgentOfficeSell}){
  return{assignAgentOfficeSell}
}
export default connect(mapStateToProps)(Form.create({})(AssignAgentOfficeSell));
