...
function HouseImgsRent({dispatch,houseImgsRent}){
  const {
    showPicList,promptObj,loadingShadow,picListData,
    } = houseImgsRent;

  var {houseBaseInfo} = houseImgsRent;
  ...
  const upLoadPicProps={
    ...
    changeList:(data)=>{
      dispatch({
        type:"houseImgsRent/showPicList",
        payload:data
      })
    },//更新list回调
  }
  ...
  const onOkCallBack=()=>{
      if(promptObj.todo==='closeModal'){
        dispatch({
          type:"houseImgsRent/togglePrompt",
          payload:{
            visible:false
          }
        })
...
function mapStateToProps({houseImgsRent}){
  return{houseImgsRent}
}
export default connect(mapStateToProps)(HouseImgsRent)
