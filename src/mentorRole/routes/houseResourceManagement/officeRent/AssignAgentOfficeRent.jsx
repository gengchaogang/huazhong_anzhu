...
import './assignAgentOfficeRent.css'
...
function AssignAgentOfficeRent({dispatch,assignAgentOfficeRent,form}){
  ...
  const onExpand = (expandedKeys) => {
      dispatch({
        type:"assignAgentOfficeRent/setStatePramas",
        payload:{
          expandedKeys,
    ...
    const checkedTreeNode=(checkedKeys,e)=>{  //勾选
      ...
      dispatch({
        type:"assignAgentOfficeRent/saveAgentTableData",
        payload:{
  ...
  const onDelete=(text,record,index)=>{
    if (slectedBrokers != null){
        ...
        dispatch({
          type:"assignAgentOfficeRent/setState",
          payload:{
  ...
  const showModal=()=>{
    dispatch({
      type:"assignAgentOfficeRent/currentSelect",
      payload:{
        agentListTableData:agentListTableData,
    ...
    // dispatch({
    //   type:"assignAgentOfficeRent/changeModal",
    //   payload:{
    //     visible:true
    //   }
    // });
    // dispatch({
    //   type:'assignAgentOfficeRentModal/getBrokerList',
    //   payload:{
    //     pageNo:0,
  ...
  const onModalOk=(data)=>{
    ...
    // 保存选择的经纪人信息
    dispatch({
        type:"assignAgentOfficeRent/addSelectBroker",
        payload:data,
    });
  }
  // 选择经纪人取消
  ...
  const onOkCallBack=()=>{
      if(promptObj.todo==='closeModal'){
        dispatch({
          type:"assignAgentOfficeRent/togglePrompt",
          payload:{
            visible:false
          }
        })
          ...
          setTimeout(function(){
            dispatch({
              type:"assignAgentOfficeRent/backlist",
            })
          }, 200)
      }
  }
  ...
  const toBack=()=>{
    dispatch(routerRedux.push({
      pathname:'/houseResourceRentManagement/officeRent/createOfficeRentResource/officeOwnerInfosRent',
    ...
    form.validateFields((err, fieldsValue) => {
     ...
     if (err) {
       ...
       if(!!fieldsValue.commissionRate&&fieldsValue.commissionRate!==''){
        ...
        dispatch({
          type:"assignAgentOfficeRent/submitData",
          payload:fieldsValue,
        });
     }
   });
  ...
  const openPromotionCheckBox=(e)=>{
      ...
      if (isChecked) {
        ...
        // 关闭合作促销
        dispatch({
          type:"assignAgentOfficeRent/clearCxInfo",
        });
        form.setFieldsValue({
  ...
  const onInputChange=(e)=>{
    ...
    dispatch({
      type:"assignAgentOfficeRent/closeCommissionAmount",
      payload:{
  ...
  const onRadioGroupChange=(e)=>{
    ...
    dispatch({
      type:"assignAgentOfficeRent/saveCurrentRadio",
      payload:{
                ...
                <Col sm={24} md={24}>
                  <FormItem label="房源销售总佣金" {...formItemLayout}>
                    {getFieldDecorator('tradingCommissions', {
                      initialValue: !!assignAgentOfficeRentObj.tradingCommissions?assignAgentOfficeRentObj.tradingCommissions.toString():null,
                      rules: [
                              ...
                              <Radio style={radioStyle} value='佣金金额'>佣金金额
                                <FormItem className="commissionType">
                                    {getFieldDecorator('commissionAmount', {
                                      initialValue: assignAgentOfficeRentObj.promotionMode==='金额'?assignAgentOfficeRentObj.commissionAmount.toString():null,
                                      rules: [
                              ...
                              <Radio style={radioStyle} value='佣金比例'>佣金比例
                                    <FormItem className="commissionType">
                                        {getFieldDecorator('commissionRate', {
                                          initialValue: !!assignAgentOfficeRentObj.commissionRate?(assignAgentOfficeRentObj.commissionRate*100).toString():null,
                                          rules: [
...
function mapStateToProps({assignAgentOfficeRent}){
  return{assignAgentOfficeRent}
}
export default connect(mapStateToProps)(Form.create({})(AssignAgentOfficeRent));
