...
function OfficeImgsRent({ dispatch, officeImgsRent }) {
  const {
    showPicList,
    promptObj,
    loadingShadow,
  ...
  var { houseBaseInfo } = officeImgsRent;
  ...
  const upLoadPicProps = {
    ...
    changeList: (data) => {
      dispatch({
        type: "officeImgsRent/deletePic",
        payload: data
      })
      dispatch({
        type: "officeImgsRent/showPicList",
        payload: data
      })
    },//更新list回调
  }
  ...
  const toNext = () => {
    if (showPicList.length) {
      dispatch({
        type: 'officeImgsRent/submitData',
        payload: {
        },
      });
    } else {
  ...
  const onOkCallBack = () => {
    if (promptObj.todo === 'closeModal') {
      dispatch({
        type: "officeImgsRent/togglePrompt",
        payload: {
          visible: false
        }
      })
  ...
  /** 创建已有的图片信息 */
  // const createUpdatePic=()=>{
  //   if (picListData != undefined) {
...
function mapStateToProps({ officeImgsRent }) {
  return { officeImgsRent }
}
export default connect(mapStateToProps)(OfficeImgsRent)
