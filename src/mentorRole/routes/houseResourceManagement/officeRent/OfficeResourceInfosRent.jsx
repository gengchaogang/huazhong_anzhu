...
function OfficeResourceInfosRent({ dispatch, form, officeResourceInfosRent }) {
  ...
  const checkHouseUnique = (value) => {
    ...
    dispatch({
      type: "officeResourceInfosRent/saveHouseUnique",
      payload: {
    ...
    dispatch({
      type: "officeResourceInfosRent/checkHouseUnique",
      payload: {
        buildingId: buildingId,
        communityId: communityId,
        houseNumberId: houseNumberId,
  ...
  const handleSubmit = (e) => {
    ...
    form.validateFields((err, fieldsValue) => {
      if (err) {
        ...
        dispatch({
          type: 'officeResourceInfosRent/submitData',
          payload: fieldsValue,
        });
      }
    })
  ...
  const onSelectChange = (value, selectedOptions) => {
    ...
    // if (value.length) {
      dispatch({
        type: "officeResourceInfosRent/changeDisabled",
        payload: {
    ...
    dispatch({
      type: 'officeResourceInfosRent/setState',
      payload: {
        areaName: areaName,
    ...
    dispatch({
      type: "officeResourceInfosRent/searchCommunities",
      payload: {
        areaName: areaName,
        type: "商铺"
      }
  ...
  const tagsChange = (checked) => {
    dispatch({
      type: "officeResourceInfosRent/changeTagsValue",
      payload: {
        tagsValue: checked
      }
    })
  ...
  const onOkCallBack = () => {
    if (promptObj.todo === 'closeModal') {
      dispatch({
        type: "officeResourceInfosRent/togglePrompt",
        payload: {
          visible: false
        }
      })
  ...
  const handleSelectBlur = (value) => {
    ...
    if (communityData.length) {
      communityData.map((item, index) => {
        if (item.name === value) {
          dispatch({
            type: "officeResourceInfosRent/getBuildingsData",
            payload: {
              id: item.id
            }
          })
  ...
  const handleBuildingNameSelect = (value, option) => {
    dispatch({
      type: "officeResourceInfosRent/getRoomsData",
      payload: {
        id: value
      }
    })
...
function mapStateToProps({ officeResourceInfosRent }) {
  return { officeResourceInfosRent }
}

export default connect(mapStateToProps)(Form.create({})(OfficeResourceInfosRent));
