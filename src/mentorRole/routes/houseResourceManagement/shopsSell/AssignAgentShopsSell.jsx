...
import './assignAgentShopsSell.css'
...
function AssignAgentShopsSell({dispatch,assignAgentShopsSell,form}){
  ...
  const onExpand = (expandedKeys) => {
      dispatch({
        type:"assignAgentShopsSell/setStatePramas",
        payload:{
          expandedKeys,
    ...
    const checkedTreeNode=(checkedKeys,e)=>{  //勾选
      ...
      dispatch({
        type:"assignAgentShopsSell/saveAgentTableData",
        payload:{
  ...
  const onDelete=(text,record,index)=>{
    if (slectedBrokers != null){
        ...
        dispatch({
          type:"assignAgentShopsSell/setState",
          payload:{
  ...
  const showModal=()=>{
    dispatch({
      type:"assignAgentShopsSell/currentSelect",
      payload:{
        agentListTableData:agentListTableData,
    ...
    // dispatch({
    //   type:"assignAgentShopsSell/changeModal",
    //   payload:{
    //     visible:true
    //   }
    // });
    // dispatch({
    //   type:'assignAgentShopsSell/getBrokerList',
    //   payload:{
    //     pageNo:0,
  ...
  const onModalOk=(data)=>{
    ...
    // 保存选择的经纪人信息
    dispatch({
        type:"assignAgentShopsSell/addSelectBroker",
        payload:data,
    });
  }
  // 选择经纪人取消
  ...
  const onOkCallBack=()=>{
      if(promptObj.todo==='closeModal'){
        dispatch({
          type:"assignAgentShopsSell/togglePrompt",
          payload:{
            visible:false
          }
        })
          ...
          setTimeout(function(){
            dispatch({
              type:"assignAgentShopsSell/backlist",
            })
          }, 200)
      }
  }
    ...
    form.validateFields((err, fieldsValue) => {
     ...
     if (err) {
       ...
       if(!!fieldsValue.commissionRate&&fieldsValue.commissionRate!==''){
        ...
        dispatch({
          type:"assignAgentShopsSell/submitData",
          payload:fieldsValue,
        });
     }
   });
  ...
  const openPromotionCheckBox=(e)=>{
      ...
      if (isChecked) {
        ...
        // 关闭合作促销
        dispatch({
          type:"assignAgentShopsSell/clearCxInfo",
        });
        form.setFieldsValue({
  ...
  const onInputChange=(e)=>{
    ...
    dispatch({
      type:"assignAgentShopsSell/closeCommissionAmount",
      payload:{
  ...
  const onRadioGroupChange=(e)=>{
    ...
    dispatch({
      type:"assignAgentShopsSell/saveCurrentRadio",
      payload:{
                ...
                <Col sm={24} md={24}>
                  <FormItem label="房源销售总佣金" {...formItemLayout}>
                    {getFieldDecorator('tradingCommissions', {
                      initialValue: !!assignAgentShopsSellObj.tradingCommissions?assignAgentShopsSellObj.tradingCommissions.toString():null,
                      rules: [
                ...
                <Col sm={24} md={24}>
                    <FormItem label="是否开启房源速销" {...formItemLayout}>
                      {getFieldDecorator('cooperativePromotion', {
                        ...
                        rules: [
                          { required: false, message:'请选择合作促销!' },
                      ],
                      })(
                              ...
                              <Radio style={radioStyle} value='佣金金额'>佣金金额
                                <FormItem className="commissionType">
                                    {getFieldDecorator('commissionAmount', {
                                      initialValue: assignAgentShopsSellObj.promotionMode==='金额'?assignAgentShopsSellObj.commissionAmount.toString():null,
                                      rules: [
                              ...
                              <Radio style={radioStyle} value='佣金比例'>佣金比例
                                    <FormItem className="commissionType">
                                        {getFieldDecorator('commissionRate', {
                                          initialValue: !!assignAgentShopsSellObj.commissionRate?(assignAgentShopsSellObj.commissionRate*100).toString():null,
                                          rules: [
...
function mapStateToProps({assignAgentShopsSell}){
  return{assignAgentShopsSell}
}
export default connect(mapStateToProps)(Form.create({})(AssignAgentShopsSell));
