...
export default {
  namespace:'remindMsgManagement',
  state:initState,
  subscriptions: {
   setup({ dispatch, history }) {
     history.listen(location => {
       if (location.pathname === '/remindMsgManagement') {
  ...
  effects:{
    ...
    *getMsgList({payload},{call,select,put}){
      const {tableLoading,pagination:{current}}=yield select(({remindMsgManagement})=>remindMsgManagement);
      if(!tableLoading){
        yield put({
    ...
    *markMsgRead({payload},{call,put}){
      ...
      if(!noRead){
        yield put(routerRedux.push({
          pathname:'/remindMsgManagement/remindMsgInfo',
          state:{
            msgId:listCode,
          }
        }))
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {todo} = yield select(({ remindMsgManagement }) => remindMsgManagement.promptObj);
      switch (todo) {
        case 'default':
