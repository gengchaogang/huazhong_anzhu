...
export default {
  ...
  subscriptions: { //路由监听
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/houseResourceRentManagement/shopsRent') {
  ...
  effects: { //发起异步请求
    ...
    *entrust({ payload }, { put, call, select }) {
      const currentState = yield select(({ mentorShopsRent }) => mentorShopsRent);
      const responseObj = yield call(requestApi, {
