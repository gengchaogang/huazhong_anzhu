...
const initState={
  ...
  houseBaseInfo:{},
  slectedBrokers:[],
  assignAgentShopsRentObj:{},              // 合作促销信息

  openPromotionCheked:false,  // 合作促销 是否已选择
  jeCheckBoxChecked:false,    // 金额 是否选择
  commissionAmount:0.0,       // 金额
  blCheckBoxChecked:false,    // 佣金比例是否选择
}
export default {
  namespace: 'assignAgentShopsRent',
  state:initState,
  reducers: {
  ...
  effects:{
    *initData({payload},{call,put,select}){
        const {houseBaseInfo} = yield select(({assignAgentShopsRent})=>assignAgentShopsRent);
        yield put({
            type:"initUpdate"
        })
    },
        ...
        if (houseBaseInfo['id']) {
            ...
            if(reObj.isSuccess) {
              ...
              reObj.listBrokers.map((item,index)=>{
               ...
               reObj.listBrokers=slectedBrokers;
                yield put({
                    type:"setState",
                    payload:{
                        assignAgentShopsRentObj: reObj,
                    }
                })
                if (reObj.listBrokers) {
                    yield put({
                      type:"saveListBroker",
                      payload:{
                        slectedBrokers: reObj.listBrokers,
                      }
                    })
                }
    ...
    *addSelectBroker({payload},{call,put,select}){
        if(payload['slectedBrokers']){
            // 已有的数据
            const {slectedBrokers} = yield select(({assignAgentShopsRent})=>assignAgentShopsRent);
    ...
    *submitData({payload},{call,put,select}){
        const _assignAgentShopsRentState = yield select(({assignAgentShopsRent})=>assignAgentShopsRent);
        const {houseBaseInfo} = _assignAgentShopsRentState;    // 房源信息
        const {slectedBrokers} = _assignAgentShopsRentState;   // 选择的经纪人信息
        ...
        if (isNaN(tradingCommissions) || tradingCommissions <= 0) {
            message.error('请输入房源佣金总金额')
            return;
        }
        submitObj['tradingCommissions'] = tradingCommissions;

        const {openPromotionCheked} = _assignAgentShopsRentState;    //  开启合作促销
        let cooperationPromotion = "关闭";
        if (openPromotionCheked) {
            cooperationPromotion = "开启";

            const {jeCheckBoxChecked} = _assignAgentShopsRentState;    // 开启金额
            if(jeCheckBoxChecked) {
                ...
                const {blCheckBoxChecked} = _assignAgentShopsRentState;                // 比例
        ...
        if(reObj.isSuccess) {
          ...
          houseBaseInfo.assignAgentShopsRentInfos = submitObj;
          yield put({
            type:"setState",
            payload:{
  ...
  subscriptions:{
    setup({ dispatch, history }) {
         history.listen(location =>{
           if (location.pathname === '/houseResourceRentManagement/shopsRent/createShopsRentResource/assignAgentShopsRent') {
             dispatch({
