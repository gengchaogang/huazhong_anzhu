...
export default {
  ...
  effects: {
    *initData({ payload }, { call, put, select }) {
      const { houseBaseInfo } = yield select(({ shopsImgsRent }) => shopsImgsRent);
      yield put({
        type: "setState",
        payload: {
          picListData: [],
    ...
    *deletePic({ payload }, { call, put, select }) {
      const currentPicList = yield select(({ shopsImgsRent }) => shopsImgsRent.showPicList);
    ...
    *submitData({ payload }, { call, put, select }) {
      ...
      const _shopsImgsRentState = yield select(({ shopsImgsRent }) => shopsImgsRent);
      var { houseBaseInfo } = _shopsImgsRentState;
      const { showPicList } = _shopsImgsRentState;
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/houseResourceRentManagement/shopsRent/createShopsRentResource/shopsImgsRent') {
          dispatch({
