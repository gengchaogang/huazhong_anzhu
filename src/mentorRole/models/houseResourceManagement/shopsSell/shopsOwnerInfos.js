...
export default {
  ...
  effects:{
    *initData({payload},{call,put,select}){
        const {houseBaseInfo} = yield select(({shopsOwnerInfos})=>shopsOwnerInfos);
        yield put ({
            type:"initUpdate"
        })
    },
    ...
    *submitData({payload},{call,put,select}){
        ...
        const _shopsOwnerInfosState = yield select(({shopsOwnerInfos})=>shopsOwnerInfos);

        const {houseBaseInfo} = _shopsOwnerInfosState;   // 房源信息
        const {houseOwerInfo} = _shopsOwnerInfosState;   // 业主信息

        const {showIdPicList} = _shopsOwnerInfosState;
        const {showHouseNuPicList} = _shopsOwnerInfosState;
        const {showContractPicList} = _shopsOwnerInfosState;
        ...
        if(reObj.isSuccess) {
          ...
          houseBaseInfo.shopsOwnerInfos = submitObj;
          yield put({
            type:"setState",
            payload:{
          ...
          //pathname:'/houseResourceSaleManagement/shopsSell/createShopsSellResource/assignAgentShopsSell',
  ...
  subscriptions:{
    setup({ dispatch, history }) {
         history.listen(location =>{
           if (location.pathname === '/houseResourceSaleManagement/shopsSell/createShopsSellResource/shopsOwnerInfos') {
             dispatch({
