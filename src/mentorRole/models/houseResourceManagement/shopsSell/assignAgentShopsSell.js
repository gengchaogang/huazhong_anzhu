...
const initState={
  ...
  houseBaseInfo:{},
  slectedBrokers:[],
  assignAgentShopsSellObj:{},              // 合作促销信息
...
export default {
  namespace: 'assignAgentShopsSell',
  state:initState,
  reducers: {
  ...
  effects:{
    *initData({payload},{call,put,select}){
        const {houseBaseInfo} = yield select(({assignAgentShopsSell})=>assignAgentShopsSell);
        yield put({
            type:"initUpdate"
        })
    },
    ...
    *addSelectBroker({payload},{call,put,select}){
        if(payload['slectedBrokers']){
            // 已有的数据
            const {slectedBrokers} = yield select(({assignAgentShopsSell})=>assignAgentShopsSell);
    ...
    *submitData({payload},{call,put,select}){
        const _assignAgentShopsSellState = yield select(({assignAgentShopsSell})=>assignAgentShopsSell);
        const {houseBaseInfo} = _assignAgentShopsSellState;    // 房源信息
        const {slectedBrokers} = _assignAgentShopsSellState;   // 选择的经纪人信息
        ...
        const {openPromotionCheked} = _assignAgentShopsSellState;    //  开启合作促销
        let cooperationPromotion = "关闭";
        if (openPromotionCheked) {
            cooperationPromotion = "开启";

            const {jeCheckBoxChecked} = _assignAgentShopsSellState;    // 开启金额
            if(jeCheckBoxChecked) {
                ...
                const {blCheckBoxChecked} = _assignAgentShopsSellState;                // 比例
        ...
        if(reObj.isSuccess) {
          ...
          houseBaseInfo.assignAgentShopsSellInfos = submitObj;
          yield put({
            type:"setState",
            payload:{
  ...
  subscriptions:{
    setup({ dispatch, history }) {
         history.listen(location =>{
           if (location.pathname === '/houseResourceSaleManagement/shopsSell/createShopsSellResource/assignAgentShopsSell') {
             dispatch({
