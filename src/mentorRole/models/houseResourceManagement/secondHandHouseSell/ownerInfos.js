...
export default {
  ...
  effects:{
    *initData({payload},{call,put,select}){
        const {houseBaseInfo} = yield select(({ownerInfos})=>ownerInfos);
        yield put ({
            type:"initUpdate"
        })
    },
    ...
    *submitData({payload},{call,put,select}){
        ...
        const _ownerInfosState = yield select(({ownerInfos})=>ownerInfos);

        const {houseBaseInfo} = _ownerInfosState;   // 房源信息
        const {houseOwerInfo} = _ownerInfosState;   // 业主信息

        const {showIdPicList} = _ownerInfosState;
        const {showHouseNuPicList} = _ownerInfosState;
        const {showContractPicList} = _ownerInfosState;
        ...
        if(reObj.isSuccess) {
          ...
          houseBaseInfo.ownerInfos = submitObj;
          yield put({
            type:"setState",
            payload:{
          ...
          yield put (routerRedux.push({
            //pathname:'/houseResourceSaleManagement/secondHandHouseSell/createSecondHandSellResource/assignAgent', 过滤掉了第四步指派经纪人。
  ...
  subscriptions:{
    setup({ dispatch, history }) {
         history.listen(location =>{
           if (location.pathname === '/houseResourceSaleManagement/secondHandHouseSell/createSecondHandSellResource/ownerInfos') {
             dispatch({
