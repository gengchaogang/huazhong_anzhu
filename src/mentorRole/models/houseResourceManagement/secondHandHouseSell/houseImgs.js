...
export default {
  ...
  effects: {
    *initData({ payload }, { call, put, select }) {
      const { houseBaseInfo } = yield select(({ houseImgs }) => houseImgs);
      yield put({
        type: "setState",
        payload: {
          picListData: [],
    ...
    *submitData({ payload }, { call, put, select }) {
      ...
      const _houseImgsState = yield select(({ houseImgs }) => houseImgs);
      var { houseBaseInfo } = _houseImgsState;
      const { showPicList } = _houseImgsState;
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/houseResourceSaleManagement/secondHandHouseSell/createSecondHandSellResource/houseImgs') {
          dispatch({
