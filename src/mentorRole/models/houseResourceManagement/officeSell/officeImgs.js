...
export default {
  ...
  effects: {
    *initData({ payload }, { call, put, select }) {
      const { houseBaseInfo } = yield select(({ officeImgs }) => officeImgs);
      yield put({
        type: "setState",
        payload: {
          picListData: [],
    ...
    *deletePic({ payload }, { call, put, select }) {
      const currentPicList = yield select(({ officeImgs }) => officeImgs.showPicList);
    ...
    *submitData({ payload }, { call, put, select }) {
      ...
      const _officeImgsState = yield select(({ officeImgs }) => officeImgs);
      var { houseBaseInfo } = _officeImgsState;
      const { showPicList } = _officeImgsState;
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/houseResourceSaleManagement/officeSell/createOfficeSellResource/officeImgs') {
          dispatch({
