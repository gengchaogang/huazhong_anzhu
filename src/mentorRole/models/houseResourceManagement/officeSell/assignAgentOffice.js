...
const initState={
  ...
  houseBaseInfo:{},
  slectedBrokers:[],
  assignAgentOfficeSellObj:{},              // 合作促销信息
...
export default {
  namespace: 'assignAgentOfficeSell',
  state:initState,
  reducers: {
  ...
  effects:{
    *initData({payload},{call,put,select}){
        const {houseBaseInfo} = yield select(({assignAgentOfficeSell})=>assignAgentOfficeSell);
        yield put({
            type:"initUpdate"
        })
    },
            ...
            if(reObj.isSuccess) {
              const slectedBrokers=[];
                yield put({
                    type:"setState",
                    payload:{
                        assignAgentOfficeSellObj: reObj,
                    }
                })
    ...
    *addSelectBroker({payload},{call,put,select}){
        if(payload['slectedBrokers']){
            // 已有的数据
            const {slectedBrokers} = yield select(({assignAgentOfficeSell})=>assignAgentOfficeSell);
    ...
    *submitData({payload},{call,put,select}){
        const _assignAgentOfficeSellState = yield select(({assignAgentOfficeSell})=>assignAgentOfficeSell);
        const {houseBaseInfo} = _assignAgentOfficeSellState;    // 房源信息
        const {slectedBrokers} = _assignAgentOfficeSellState;   // 选择的经纪人信息
        ...
        const {openPromotionCheked} = _assignAgentOfficeSellState;    //  开启合作促销
        let cooperationPromotion = "关闭";
        if (openPromotionCheked) {
            cooperationPromotion = "开启";

            const {jeCheckBoxChecked} = _assignAgentOfficeSellState;    // 开启金额
            if(jeCheckBoxChecked) {
                ...
                const {blCheckBoxChecked} = _assignAgentOfficeSellState;                // 比例
        ...
        if(reObj.isSuccess) {
          ...
          houseBaseInfo.assignAgentOfficeSellInfos = submitObj;
          yield put({
            type:"setState",
            payload:{
  ...
  subscriptions:{
    setup({ dispatch, history }) {
         history.listen(location =>{
           if (location.pathname === '/houseResourceSaleManagement/officeSell/createOfficeSellResource/assignAgentOfficeSell') {
             dispatch({
