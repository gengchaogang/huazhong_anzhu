...
const typeGroups = [
  ...
  {
    "areaPath":"",
    "typeName":labelsFinalCode.labelXzlcsts,    //写字楼房源特色
  },
  {
    "areaPath":"",
    "typeName": labelsFinalCode.labelZxqk,    //写字楼房源特色
  },
];
...
const initState = {
  ...
  houseBaseInfo:{         // 房源默认信息
    resourcesType: commonFinalCode.senondHouseResourcesType_xzl,   // 商铺
  ...
  effects:{
    *checkHouseUnique({payload},{call,put}){
      ...
      *submitData({payload},{call,put,select}){
          // 区域信息
          const {houseUnique} = yield select(({officeResourceInfos})=>officeResourceInfos);
          var areaStr = "";
          if (payload['area']) {
            const area = payload['area'];
            areaStr = area.join('/');
            payload['area'] = areaStr;
            const {areaName} = yield select(({officeResourceInfos})=>officeResourceInfos);
          ...
          // 默认添加
          let paiName = "/miss-anzhu-secdhouse-resource/resource/addOfficeBuildingSell";
          let {houseBaseInfo} = yield select(({officeResourceInfos})=>officeResourceInfos);
          if (houseBaseInfo != null && houseBaseInfo['id']) {
              // 编辑
              paiName = "/miss-anzhu-secdhouse-resource/resource/updateOfficeBuildingSell";
            ...
            if(reObj.isSuccess) {
              yield put({
                ...
                yield put (routerRedux.push({
                    pathname:'/houseResourceSaleManagement/officeSell/createOfficeSellResource/officeImgs',
  ...
  subscriptions:{
    setup({ dispatch, history }) {
         history.listen(location =>{
           if (location.pathname === '/houseResourceSaleManagement/officeSell/createOfficeSellResource/officeResourceInfos') {
               dispatch({
                 type: 'initState',
               });
               dispatch({
