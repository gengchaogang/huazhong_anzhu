...
export default {
  ...
  subscriptions: { //路由监听
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/houseResourceSaleManagement/officeSell') {
  ...
  effects: { //发起异步请求
    ...
    *entrust({ payload }, { put, call, select }) {
      const currentState = yield select(({ mentorOfficeSell }) => mentorOfficeSell);
      const responseObj = yield call(requestApi, {
