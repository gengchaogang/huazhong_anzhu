...
export default {
  ...
  effects:{
    *initData({payload},{call,put,select}){
        const {houseBaseInfo} = yield select(({ownerInfosRent})=>ownerInfosRent);
    ...
    *submitData({payload},{call,put,select}){
        ...
        const _ownerInfosRentState = yield select(({ownerInfosRent})=>ownerInfosRent);

        const {houseBaseInfo} = _ownerInfosRentState;   // 房源信息
        const {houseOwerInfo} = _ownerInfosRentState;   // 业主信息
        const {currentIdentity} = _ownerInfosRentState;   // 业主身份

        const {showIdPicList} = _ownerInfosRentState;
        const {showHouseNuPicList} = _ownerInfosRentState;
        const {leaseContractPicList} = _ownerInfosRentState; //租赁合同附件
        const {showContractPicList} = _ownerInfosRentState;
        ...
        if(reObj.isSuccess) {
          ...
          houseBaseInfo.ownerInfosRent = submitObj;
          yield put({
            type:"setState",
            payload:{
  ...
  subscriptions:{
    setup({ dispatch, history }) {
         history.listen(location =>{
           if (location.pathname === '/houseResourceRentManagement/secondHandHouseRent/createSecondHandRentResource/ownerInfosRent') {
             dispatch({
