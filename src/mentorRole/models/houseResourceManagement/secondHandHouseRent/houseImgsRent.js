...
export default {
  ...
  effects: {
    *initData({ payload }, { call, put, select }) {
      const { houseBaseInfo } = yield select(({ houseImgsRent }) => houseImgsRent);
      yield put({
        type: "setState",
        payload: {
          picListData: [],
    ...
    *deletePic({ payload }, { call, put, select }) {
      const currentPicList = yield select(({ houseImgsRent }) => houseImgsRent.showPicList);
    ...
    *submitData({ payload }, { call, put, select }) {
      ...
      const _houseImgsRentState = yield select(({ houseImgsRent }) => houseImgsRent);
      var { houseBaseInfo } = _houseImgsRentState;
      const { showPicList } = _houseImgsRentState;
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/houseResourceRentManagement/secondHandHouseRent/createSecondHandRentResource/houseImgsRent') {
          dispatch({
