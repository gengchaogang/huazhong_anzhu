...
export default {
  ...
  reducers: {
    ...
    saveCurrentRadioValue(state,action){
      return{...state,...action.payload}
    },
  ...
  effects:{
    *initData({payload},{call,put,select}){
        const {houseBaseInfo} = yield select(({assignAgentRent})=>assignAgentRent);
        yield put({
            type:"initUpdate"
        })
    },
    ...
    *addSelectBroker({payload},{call,put,select}){
        if(payload['slectedBrokers']){
            // 已有的数据
            const {slectedBrokers} = yield select(({assignAgentRent})=>assignAgentRent);
    ...
    *submitData({payload},{call,put,select}){
        const _assignAgentRentState = yield select(({assignAgentRent})=>assignAgentRent);
        const {houseBaseInfo} = _assignAgentRentState;    // 房源信息
        const {slectedBrokers} = _assignAgentRentState;   // 选择的经纪人信息
        ...
        const {openPromotionCheked} = _assignAgentRentState;    //  开启合作促销
        let cooperationPromotion = "关闭";
        if (openPromotionCheked) {
            cooperationPromotion = "开启";

            const {jeCheckBoxChecked} = _assignAgentRentState;    // 开启金额
            if(jeCheckBoxChecked) {
                ...
                const {blCheckBoxChecked} = _assignAgentRentState;                // 比例
        ...
        if(reObj.isSuccess) {
          ...
          houseBaseInfo.assignAgentRentInfos = submitObj;
          yield put({
            type:"setState",
            payload:{
  ...
  subscriptions:{
    setup({ dispatch, history }) {
         history.listen(location =>{
           if (location.pathname === '/houseResourceRentManagement/secondHandHouseRent/createSecondHandRentResource/assignAgentRent') {
             dispatch({
