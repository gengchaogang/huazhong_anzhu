...
export default {
  ...
  effects: { //发起异步请求
    ...
    *getEopOptions({ payload }, { put, call }) {
      ...
      const reObj2 = analysisUtil.analysisDataResponse(labelsResponseObj);
      if (reObj2.isSuccess) {
        const labels = commonUtil.ansyslabelsData(reObj2.content);
        yield put({
          type: 'saveResultData',
          payload: {
            labels: labels
    ...
    *loadList({ payload }, { put, call, select }) {
      ...
      if (activeKey === 'publishedHouse') {
        ...
        const { pausePage } = mentorSecondHandHouseRent;
        pageObj = pausePage;
      }
    ...
    *entrust({ payload }, { put, call, select }) {
      const currentState = yield select(({ mentorSecondHandHouseRent }) => mentorSecondHandHouseRent);
      const responseObj = yield call(requestApi, {
