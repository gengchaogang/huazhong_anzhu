...
const initState={
  ...
  houseBaseInfo:{},
  slectedBrokers:[],
  assignAgentOfficeRentObj:{},              // 合作促销信息
...
export default {
  namespace: 'assignAgentOfficeRent',
  state:initState,
  reducers: {
  ...
  effects:{
    *initData({payload},{call,put,select}){
        const {houseBaseInfo} = yield select(({assignAgentOfficeRent})=>assignAgentOfficeRent);
        yield put({
            type:"initUpdate"
        })
    },
            ...
            if(reObj.isSuccess) {
              const slectedBrokers=[];
                yield put({
                    type:"setState",
                    payload:{
                        assignAgentOfficeRentObj: reObj,
    ...
    *addSelectBroker({payload},{call,put,select}){
        if(payload['slectedBrokers']){
            // 已有的数据
            const {slectedBrokers} = yield select(({assignAgentOfficeRent})=>assignAgentOfficeRent);
    ...
    *submitData({payload},{call,put,select}){
        const _assignAgentOfficeRentState = yield select(({assignAgentOfficeRent})=>assignAgentOfficeRent);
        const {houseBaseInfo} = _assignAgentOfficeRentState;    // 房源信息
        const {slectedBrokers} = _assignAgentOfficeRentState;   // 选择的经纪人信息
        ...
        const {openPromotionCheked} = _assignAgentOfficeRentState;    //  开启合作促销
        let cooperationPromotion = "关闭";
        if (openPromotionCheked) {
            cooperationPromotion = "开启";

            const {jeCheckBoxChecked} = _assignAgentOfficeRentState;    // 开启金额
            if(jeCheckBoxChecked) {
                ...
                const {blCheckBoxChecked} = _assignAgentOfficeRentState;                // 比例
        ...
        if(reObj.isSuccess) {
          ...
          houseBaseInfo.assignAgentOfficeRentInfos = submitObj;
          yield put({
            type:"setState",
            payload:{
  ...
  subscriptions:{
    setup({ dispatch, history }) {
         history.listen(location =>{
           if (location.pathname === '/houseResourceRentManagement/officeRent/createOfficeRentResource/assignAgentOfficeRent') {
             dispatch({
