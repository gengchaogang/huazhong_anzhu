...
export default {
  namespace: 'officeOwnerInfosRent',
  state:initState,
  reducers: {
  ...
  effects:{
    *initData({payload},{call,put,select}){
        const {houseBaseInfo} = yield select(({officeOwnerInfosRent})=>officeOwnerInfosRent);
        yield put ({
            type:"initUpdate"
        })
    },
    ...
    *submitData({payload},{call,put,select}){
        ...
        const _officeOwnerInfosRentState = yield select(({officeOwnerInfosRent})=>officeOwnerInfosRent);

        const {houseBaseInfo} = _officeOwnerInfosRentState;   // 房源信息
        const {houseOwerInfo} = _officeOwnerInfosRentState;   // 业主信息

        const {showIdPicList} = _officeOwnerInfosRentState;
        const {showHouseNuPicList} = _officeOwnerInfosRentState;
        const {showContractPicList} = _officeOwnerInfosRentState;
        const {leaseContractPicList} = _officeOwnerInfosRentState;
        const {currentIdentity} = _officeOwnerInfosRentState;
        ...
        if(reObj.isSuccess) {
          ...
          houseBaseInfo.officeOwnerInfosRent = submitObj;
          yield put({
            type:"setState",
            payload:{
          ...
          yield put (routerRedux.push({
            ...
            //pathname:'/houseResourceRentManagement/officeRent/createOfficeRentResource/assignAgentOfficeRent',
  ...
  subscriptions:{
    setup({ dispatch, history }) {
         history.listen(location =>{
           if (location.pathname === '/houseResourceRentManagement/officeRent/createOfficeRentResource/officeOwnerInfosRent') {
             dispatch({
