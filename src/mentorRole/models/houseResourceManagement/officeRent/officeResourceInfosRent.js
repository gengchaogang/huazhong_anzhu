...
export default {
  namespace: 'officeResourceInfosRent',
  state: initState,
  reducers: {
  ...
  effects:{
    ...
    *submitData({payload},{call,put,select}){
      const {houseUnique} = yield select(({officeResourceInfosRent})=>officeResourceInfosRent);
        // 区域信息
        var areaStr = "";
        if (payload['area']) {
          ...
          const {areaName} = yield select(({officeResourceInfosRent})=>officeResourceInfosRent);
        ...
        let {houseBaseInfo} = yield select(({officeResourceInfosRent})=>officeResourceInfosRent);
