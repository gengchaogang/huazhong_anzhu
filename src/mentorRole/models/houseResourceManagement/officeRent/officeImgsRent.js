...
export default {
  ...
  effects: {
    *initData({ payload }, { call, put, select }) {
      const { houseBaseInfo } = yield select(({ officeImgsRent }) => officeImgsRent);
      yield put({
        type: "setState",
        payload: {
          picListData: [],
    ...
    *deletePic({ payload }, { call, put, select }) {
      const currentPicList = yield select(({ officeImgsRent }) => officeImgsRent.showPicList);
    ...
    *submitData({ payload }, { call, put, select }) {
      ...
      const _officeImgsRentState = yield select(({ officeImgsRent }) => officeImgsRent);
      var { houseBaseInfo } = _officeImgsRentState;
      const { showPicList } = _officeImgsRentState;
      ...
      addImagsObj.updateAllFile = '是';//是否替換以前的圖片
      // 上传房源图片文件
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/houseResourceRentManagement/officeRent/createOfficeRentResource/officeImgsRent') {
          dispatch({
