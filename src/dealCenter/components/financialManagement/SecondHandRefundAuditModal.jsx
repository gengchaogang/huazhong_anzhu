...
function SecondHandRefundAuditModal({dispatch, secondHandHouseFinancialSalesAudit}){
	const {refundAuditModal, auditExplainModal,resultTableData,resultReason,resultInfos} = secondHandHouseFinancialSalesAudit;
			...
			payload:{
				...
				onSubmit:(values)=>{
					...
					if(currentRecord.refundType==="购房定金"){
						dispatch({
							type:"secondHandHouseFinancialSalesAudit/intentMoneyPass",
							payload:values
						})
					}else if(currentRecord.refundType==="首付款"){
						dispatch({
							type:"secondHandHouseFinancialSalesAudit/downPaymentPass",
							payload:values
						})
					}else if(currentRecord.refundType==="佣金"){
						dispatch({
							type:"secondHandHouseFinancialSalesAudit/commissionPass",
							payload:values
						})
					}
				}
		...
		dispatch({
			...
			payload:{
				...
				onSubmit:(values)=>{
					if(currentRecord.refundType==="购房定金"){
						dispatch({
							type:"secondHandHouseFinancialSalesAudit/intentMoneyReject",
							payload:values
						})
					}else if(currentRecord.refundType==="首付款"){
						dispatch({
							type:"secondHandHouseFinancialSalesAudit/downPaymentReject",
							payload:values
						})
					}else if(currentRecord.refundType==="佣金"){
						dispatch({
							type:"secondHandHouseFinancialSalesAudit/commissionReject",
							payload:values
						})
					}
				},
...
SecondHandRefundAuditModal.propTypes={
	// secondHandHouseFinancialSalesAudit:PropTypes.object.isRequired,
};
function mapStateToProps({ secondHandHouseFinancialSalesAudit }) {
	return { secondHandHouseFinancialSalesAudit };
}
export default connect(mapStateToProps)(SecondHandRefundAuditModal);
