...
function SecondHandRentRefundAuditModal({dispatch, secondHandHouseRentalAudit}){
	const {refundAuditModal, auditExplainModal,resultTableData,resultReason,resultInfos} = secondHandHouseRentalAudit;
	...
	const handleCancel=()=>{
		dispatch({
			type:'secondHandHouseRentalAudit/toggleRefundAuditModal',
			payload:{
				visible:false,
				}
		});
				...
				payload:{
					...
					onSubmit:(values)=>{
						...
						if(currentRecord.refundType==="购房定金"){
							dispatch({
								type:"secondHandHouseRentalAudit/intentMoneyPass",
								payload:values
							})
						}else if(currentRecord.refundType==="首付款"){
							dispatch({
								type:"secondHandHouseRentalAudit/downPaymentPass",
								payload:values
							})
						}else if(currentRecord.refundType==="佣金"){
							dispatch({
								type:"secondHandHouseRentalAudit/commissionPass",
								payload:values
							})
						}
					}
		...
		dispatch({
			...
			payload:{
				...
				onSubmit:(values)=>{
					if(currentRecord.refundType==="购房定金"){
						dispatch({
							type:"secondHandHouseRentalAudit/intentMoneyReject",
							payload:values
						})
					}else if(currentRecord.refundType==="首付款"){
						dispatch({
							type:"secondHandHouseRentalAudit/downPaymentReject",
							payload:values
						})
					}else if(currentRecord.refundType==="佣金"){
						dispatch({
							type:"secondHandHouseRentalAudit/commissionReject",
							payload:values
						})
					}
				},
...
SecondHandRentRefundAuditModal.propTypes={
	// secondHandHouseRentalAudit:PropTypes.object.isRequired,
};
