...
function ProjectDealAuditModal({dispatch, newHouseTransactionContractReview}){
	const {projectDealAuditModal, auditExplainModal, trackDetailData} = newHouseTransactionContractReview;
	...
	const columns=[{
		...
		render:(text,record)=>{
			return(
				<a style={{color:'#009900'}} onClick={()=>toDealDetail(record.groupKey)}>交易详情</a>
			)
		}
	}];
	...
	// 		const images=currentRecord.projectQualification.map(item=>({
	// 			title:'',
	// 			src:item.url,
	// 			isCover:false,
	// 		}))
	// 		return <PicList picListData={images}/>
	// 		// return (
		...
		dispatch({
			...
			payload:{
				...
				onSubmit:(values)=>{
					...
					values['id'] = id;
					dispatch({
							type:"newHouseTransactionContractReview/newHouseDealPass",
							payload:values
					})
				}
			}
		...
		dispatch({
			...
			payload:{
				...
				onSubmit:(values)=>{
					...
					values['id'] = id;
					dispatch({
							type:"newHouseTransactionContractReview/newhouseRejectDealAudit",
							payload:values
					})
				},
			}
	...
	*/
	...
	// }
	let images = [];
	if (currentRecord !== null && !!currentRecord.attachments) {
		console.log('currentRecord',currentRecord);
		images=parseJSONArray(currentRecord.attachments).map(item=>({
			title:'',
			src:item,
			isCover:false,
		}))
		...
		if(visible && currentRecord){
			return(
				<div className="modalDivTop">
					...
					{images.length!==0 && <PicList picListData={images}/>}
					{/*<ul className="imgUl">
						{
							images.map((item)=>{
								return (
