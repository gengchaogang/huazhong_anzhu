...
import { Modal, Button, Table, message,Timeline, } from 'antd';
...
function NewHouseRefundAuditModal({dispatch, newHouseTransactionContractReview}){
	const {refundAuditModal, auditExplainModal,refundOrerInfo,
		resultTableData,resultReason,
		resultInfos,} = newHouseTransactionContractReview;
	...
	//步骤条状态
	let stepState={current:1,status:"process"};
	if (visible) {
	...
	const columns=[{
		...
		render:(text,record)=>{
			return(
				<a style={{color:'#009900'}} onClick={
						()=>toDealDetail(record.groupKey)}>交易详情</a>
			)
		}
	}];
		...
		dispatch({
			...
			payload:{
				...
				onSubmit:(values)=>{
					...
					dispatch({
						type:"newHouseTransactionContractReview/newHouseRefundAuditPass",
						payload:values
					})
				}
			}
		...
		dispatch({
			...
			payload:{
				...
				onSubmit:(values)=>{
					...
					values['groupbuyId'] = groupbuyId;
					dispatch({
						type:"newHouseTransactionContractReview/newhouseRefundAuditRefuse",
						payload:values
					})
				},
			}
	...
	const showFooterButton=()=>{
		let footer = [];
		if(currentRecord.refundStatus=='待合同审核'){
			footer=[
		...
		<Modal
			...
			<div className="commonModalHeight">
					...
					<DxSteps current={stepState.current} status={stepState.status}>
							<DxStep description={stepState.description0} title="申请退款" />
							<DxStep description={stepState.description1} title="退款审核" />
							<DxStep description={stepState.description2} title="财务审核" />
							<DxStep description={stepState.description3} title="执行退款" />
					</DxSteps>
