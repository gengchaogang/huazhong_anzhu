...
/********************************************************/
/********************************************************/
/* 合同审核：退款  二手房出售：意向金、首付、佣金 退款审核 ***/
/********************************************************/
/********************************************************/


function SecondHandRefundAuditModal({dispatch, secondHandHouseSalesAudit}){
	...
	console.log('refundAuditModal',refundAuditModal);
	...
	},
	{
		...
		render:(text,record)=>{
			return(
				<a style={{color:'#009900'}} onClick={()=>dispatch(routerRedux.push({
				...
				payload:{
					...
					onSubmit:(values)=>{
						...
						if(currentRecord.refundType==="购房定金"){
							dispatch({
								type:"secondHandHouseSalesAudit/intentMoneyPass",
								payload:values
							})
						}else if(currentRecord.refundType==="首付款"){
							dispatch({
								type:"secondHandHouseSalesAudit/downPaymentPass",
								payload:values
							})
						}else if(currentRecord.refundType==="佣金"){
							dispatch({
								type:"secondHandHouseSalesAudit/commissionPass",
								payload:values
							})
						}
					}
		...
		dispatch({
			...
			payload:{
				...
				onSubmit:(values)=>{
					if(currentRecord.refundType==="购房定金"){
						dispatch({
							type:"secondHandHouseSalesAudit/intentMoneyReject",
							payload:values
						})
					}else if(currentRecord.refundType==="首付款"){
						dispatch({
							type:"secondHandHouseSalesAudit/downPaymentReject",
							payload:values
						})
					}else if(currentRecord.refundType==="佣金"){
						dispatch({
							type:"secondHandHouseSalesAudit/commissionReject",
							payload:values
						})
					}
				},
	...
	const showFooterButton=()=>{
		let footer=[];
		// 出租成交审核
		if (visible && _refundAuditStatus == '等待审核' && currentRecord.applyAuditStatus!=='等待财务审核') {
				footer=[
