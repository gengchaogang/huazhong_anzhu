...
const NavMenu = ({ menus,onSelectCallBack,selectKeys,openKeys,openChange}) => {
  ...
  const menuNode = menus.map((item) => {
    if(!!item.show){
      if(item.child){
        return (
          <SubMenu key={item.key} title={<span><span className={`anzhu_menu_icon centerPic  ${item.icon}`}></span>{item.title}</span>}>
            { item.child.map((ti) => {
              if(!!ti.show){
  ...
  return (
    <aside className='ant-layout-sider'>
      <div className={`ant-layout-logo centerPic menu_icon_logo`}></div>
      <Menu mode='inline'
        theme='dark'
        defaultSelectedKeys={['/']}
        onSelect={onSelect}
