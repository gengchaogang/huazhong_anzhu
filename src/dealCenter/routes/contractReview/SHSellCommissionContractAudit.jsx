...
import './SHSellCommissionContractAudit.css'
...
const SHSellCommissionContractAudit=({dispatch,shSellCommissionContractAudit:{
  promptObj,
  upLoadPicList,
  loading,
  auditorList,
  ...
  return (
    <div className='shSellCommissionContractAudit'>
      {!!loading && <DxLoadingShadow visible={!!loading}/>}
      <DxAuditModal {...auditModalProps} {...auditModal} doAudit={(data)=>dispatch({
          type:'shSellCommissionContractAudit/doAudit',
          payload:data,
        })} closeModal={()=>dispatch({
          type:'shSellCommissionContractAudit/cancelAudit'
        })} afterClose={()=>dispatch({
          type:'shSellCommissionContractAudit/afterCloseAuditModal'
        })}/>
      <PromptModal {...promptObj} onOk={()=>dispatch({type:'shSellCommissionContractAudit/closePrompt'})} onCancel={()=>dispatch({type:'shSellCommissionContractAudit/closePrompt'})}/>
      <DxPanel title='二手房出售佣金退款申请审核'>
        ...
        <div className='anzhua_button_bottom'>
          {!!canAudit && <Button type='primary' onClick={()=>dispatch({
              type:'shSellCommissionContractAudit/updateAudit',
              payload:'pass',
            })}>审核通过</Button>}
...
SHSellCommissionContractAudit.propTypes = {
  shSellCommissionContractAudit: PropTypes.object,
  location: PropTypes.object,
  dispatch: PropTypes.func,
}
function mapStateToProps({shSellCommissionContractAudit}) {
  return {shSellCommissionContractAudit}
}
export default connect(mapStateToProps)(SHSellCommissionContractAudit);
