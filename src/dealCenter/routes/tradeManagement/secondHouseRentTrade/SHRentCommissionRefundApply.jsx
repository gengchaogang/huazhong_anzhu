...
import './SHRentCommissionRefundApply.css'
const steps =[
  '申请退款',
  '退款审核',
  '财务审核',
...
const SHRentCommissionRefundApply=({dispatch,shRentCommissionRefundApply:{
  promptObj,
  upLoadPicList,
  auditorList,
  orderInfo,
  ...
  function updateApplyForm(){
    validateFields((errors) => {
      ...
      dispatch({
        type:'shRentCommissionRefundApply/postRefundApplyData',
        payload:data,
      })
    });
  }
  return (
    <div className='shRentCommissionRefundApply applyInfoCom'>
      <PromptModal {...promptObj} onOk={()=>dispatch({type:'shRentCommissionRefundApply/closePrompt'})} onCancel={()=>dispatch({type:'shRentCommissionRefundApply/closePrompt'})}/>
      <DxPanel title='二手房出租佣金退款申请'>
        ...
        <DxPanelMini title='退款金额'>
          <div style={{paddingTop:20}}>
            <FormItem
              label='退款金额'
              hasFeedback
              labelCol={{xs: 8,sm:6,md:4,lg:3}}
              wrapperCol={{xs: 8,sm:7,md:6,lg:5}}
              >
              {getFieldDecorator('refundCommissionAmount', {
                validateTrigger: 'onBlur',
                rules: [
                  {required: true, message: '佣金退款金额未填写' },
                  { type:'string',pattern:/^\d+\.?\d*$/, message: '输入内容异常' },
                ],
              })(
                <Input type='text' placeholder='请在此输入佣金退款金额' addonAfter='元'/>
              )}
            </FormItem>
          </div>
        </DxPanelMini>
        <DxPanelMini title='退款理由'>
          <div style={{paddingTop:'20px'}}>
            ...
            <DxUpLoadPic {...upLoadProps} changeList={(arr)=>dispatch({type:'shRentCommissionRefundApply/updatePicList',payload:arr})}
...
SHRentCommissionRefundApply.propTypes = {
  shRentCommissionRefundApply: PropTypes.object,
...
function mapStateToProps({shRentCommissionRefundApply}) {
  return {shRentCommissionRefundApply}
}
export default connect(mapStateToProps)(Form.create()(SHRentCommissionRefundApply));
