...
import './SHSellIntentsRefundApply.css'
const steps =[
  '申请退款',
  '退款审核',
  '财务审核',
...
const SHSellIntentsRefundApply=({dispatch,shSellIntentsRefundApply:{
  promptObj,
  upLoadPicList,
  auditorList,
  orderInfo,
  ...
  function updateApplyForm(){
    validateFields((errors) => {
      ...
      dispatch({
        type:'shSellIntentsRefundApply/postRefundApplyData',
        payload:data,
      })
    });
  }
  ...
  return (
    <div className='shSellIntentsRefundApply'>
      <PromptModal {...promptObj} onOk={()=>dispatch({type:'shSellIntentsRefundApply/closePrompt'})} onCancel={()=>dispatch({type:'shSellIntentsRefundApply/closePrompt'})}/>
      <DxPanel title='二手房出售意向金退款申请'>
        ...
        <DxPanelMini title='退款理由'>
          <div style={{paddingTop:'20px'}}>
            ...
            <DxUpLoadPic {...upLoadProps} changeList={(arr)=>dispatch({type:'shSellIntentsRefundApply/updatePicList',payload:arr})}
...
SHSellIntentsRefundApply.propTypes = {
  shSellIntentsRefundApply: PropTypes.object,
...
function mapStateToProps({shSellIntentsRefundApply}) {
  return {shSellIntentsRefundApply}
}
export default connect(mapStateToProps)(Form.create()(SHSellIntentsRefundApply));
