...
import './SHRentCommissionFinanceAudit.css'
...
const SHRentCommissionFinanceAudit=({dispatch,shRentCommissionFinanceAudit:{
  promptObj,
  upLoadPicList,
  loading,
  auditorList,
  ...
  return (
    <div className='shRentCommissionFinanceAudit applyInfoCom'>
      {!!loading && <DxLoadingShadow visible={!!loading}/>}
      <DxAuditModal {...auditModalProps} {...auditModal} doAudit={(data)=>dispatch({
          type:'shRentCommissionFinanceAudit/doAudit',
          payload:data,
        })} closeModal={()=>dispatch({
          type:'shRentCommissionFinanceAudit/cancelAudit'
        })} afterClose={()=>dispatch({
          type:'shRentCommissionFinanceAudit/afterCloseAuditModal'
        })}/>
      <PromptModal {...promptObj} onOk={()=>dispatch({type:'shRentCommissionFinanceAudit/closePrompt'})} onCancel={()=>dispatch({type:'shRentCommissionFinanceAudit/closePrompt'})}/>
      <DxPanel title='二手房出租佣金退款申请审核'>
        ...
        <div className='anzhua_button_bottom'>
          {!!canAudit && <Button type='primary' onClick={()=>dispatch({
              type:'shRentCommissionFinanceAudit/updateAudit',
              payload:'pass',
            })}>审核通过</Button>}
...
SHRentCommissionFinanceAudit.propTypes = {
  shRentCommissionFinanceAudit: PropTypes.object,
  location: PropTypes.object,
  dispatch: PropTypes.func,
}
function mapStateToProps({shRentCommissionFinanceAudit}) {
  return {shRentCommissionFinanceAudit}
}
export default connect(mapStateToProps)(SHRentCommissionFinanceAudit);
