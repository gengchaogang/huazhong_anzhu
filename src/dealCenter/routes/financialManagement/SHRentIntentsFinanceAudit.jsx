...
import './SHRentIntentsFinanceAudit.css'
...
const SHRentIntentsFinanceAudit=({dispatch,shRentIntentsFinanceAudit:{
  promptObj,
  upLoadPicList,
  loading,
  auditorList,
  ...
  return (
    <div className='shRentIntentsFinanceAudit applyInfoCom'>
      {!!loading && <DxLoadingShadow visible={!!loading}/>}
      <DxAuditModal {...auditModalProps} {...auditModal} doAudit={(data)=>dispatch({
          type:'shRentIntentsFinanceAudit/doAudit',
          payload:data,
        })} closeModal={()=>dispatch({
          type:'shRentIntentsFinanceAudit/cancelAudit'
        })} afterClose={()=>dispatch({
          type:'shRentIntentsFinanceAudit/afterCloseAuditModal'
        })}/>
      <PromptModal {...promptObj} onOk={()=>dispatch({type:'shRentIntentsFinanceAudit/closePrompt'})} onCancel={()=>dispatch({type:'shRentIntentsFinanceAudit/closePrompt'})}/>
      <DxPanel title='二手房出租意向金退款申请审核详情'>
        ...
        <div className='anzhua_button_bottom'>
          {!!canAudit && <Button type='primary' onClick={()=>dispatch({
              type:'shRentIntentsFinanceAudit/updateAudit',
              payload:'pass',
            })}>审核通过</Button>}
...
SHRentIntentsFinanceAudit.propTypes = {
  shRentIntentsFinanceAudit: PropTypes.object,
  location: PropTypes.object,
  dispatch: PropTypes.func,
}
function mapStateToProps({shRentIntentsFinanceAudit}) {
  return {shRentIntentsFinanceAudit}
}
export default connect(mapStateToProps)(SHRentIntentsFinanceAudit);
