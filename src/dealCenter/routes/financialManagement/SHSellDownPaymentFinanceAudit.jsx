...
import './SHSellDownPaymentFinanceAudit.css'
...
const SHSellDownPaymentFinanceAudit=({dispatch,shSellDownPaymentFinanceAudit:{
  promptObj,
  upLoadPicList,
  loading,
  auditorList,
  ...
  return (
    <div className='shSellDownPaymentFinanceAudit'>
      {!!loading && <DxLoadingShadow visible={!!loading}/>}
      <DxAuditModal {...auditModalProps} {...auditModal} doAudit={(data)=>dispatch({
          type:'shSellDownPaymentFinanceAudit/doAudit',
          payload:data,
        })} closeModal={()=>dispatch({
          type:'shSellDownPaymentFinanceAudit/cancelAudit'
        })} afterClose={()=>dispatch({
          type:'shSellDownPaymentFinanceAudit/afterCloseAuditModal'
        })}/>
      <PromptModal {...promptObj} onOk={()=>dispatch({type:'shSellDownPaymentFinanceAudit/closePrompt'})} onCancel={()=>dispatch({type:'shSellDownPaymentFinanceAudit/closePrompt'})}/>
      <DxPanel title='二手房出售首付款退款申请审核'>
        ...
        <div className='anzhua_button_bottom'>
          {!!canAudit && <Button type='primary' onClick={()=>dispatch({
              type:'shSellDownPaymentFinanceAudit/updateAudit',
              payload:'pass',
            })}>审核通过</Button>}
...
SHSellDownPaymentFinanceAudit.propTypes = {
  shSellDownPaymentFinanceAudit: PropTypes.object,
  location: PropTypes.object,
  dispatch: PropTypes.func,
}
function mapStateToProps({shSellDownPaymentFinanceAudit}) {
  return {shSellDownPaymentFinanceAudit}
}
export default connect(mapStateToProps)(SHSellDownPaymentFinanceAudit);
