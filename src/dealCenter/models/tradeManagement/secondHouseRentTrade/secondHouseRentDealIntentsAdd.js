...
}from '../../../services/tradeManagement/secondHouseRentTrade/secondHouseRentDealIntentsAdd'
import {
...
export default {
  namespace: 'secondHouseRentDealIntentsAdd',
  state:initState,
  subscriptions: {
   setup({ dispatch, history }) {
     history.listen(location => {
       if (location.pathname === '/tradeManagement/secondHouseRentTrade/secondHouseRentDealIntentsAdd'){
         dispatch({
           type:'doInitState',
         })
         if(!!location.state && !!location.state.transCode){
           console.log('location.state.transactionMode',location.state.transactionMode);
           dispatch({
  ...
  effects:{
        ...
        if(data.status==='success'){
          if(!!data.data.orderNumber){
            ...
            const noPayOrderInfo={
              ...
              rentPrice:renderRentMeony(intentions.actualRent,intentions.propertyType),
              payAmount:renderMoneyStr(intentions.intentionAmount),
              createDateTime:isNull(data.data.createDateTime,'-'),
              transCode:isNull(intentions.transCode,'-'),
            }
            // const intentionArr=[
            //   {
            //     label:'房屋意向租金',
            //     value:isNull(intentions.actualRent,''),
            //   },{
            //     label:'需支付的意向定金',
    ...
    *validateCustomerInfo({payload},{call,put,select}){
      const {trackJSON,transactionMode}=yield select(({secondHouseRentDealIntentsAdd})=>secondHouseRentDealIntentsAdd);
      if(transactionMode==='合作成交'){
        const {customerPhone}=payload;
        let customerBrokerId = null;
        try {
          customerBrokerId = getNewTypeTrackJSONData(trackJSON,'租户所属经纪人').id
        } catch (e) {
    ...
    *creatInentsOrder({payload},{call,put,select}){
      ...
      const{transCode}=yield select(({secondHouseRentDealIntentsAdd})=>secondHouseRentDealIntentsAdd);
      const {data}=yield call(addIntentionSellFetch,{
        actualRent:payload.actualRent,
        transCode:transCode,
        intentionAmount:payload.intentionAmount,
    ...
    *closePrompt({payload},{select,call,put}){
      const{promptObj:{todo,},transCode}=yield select(({secondHouseRentDealIntentsAdd})=>secondHouseRentDealIntentsAdd);
      switch (todo) {
        case 'default':
          yield put({
