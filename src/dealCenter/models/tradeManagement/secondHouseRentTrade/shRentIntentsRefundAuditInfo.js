...
export default {
  namespace: 'shRentIntentsRefundAuditInfo',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/tradeManagement/secondHouseRentTrade/shRentIntentsRefundAuditInfo') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *readyRevokeApply({payload},{put,select}){
      const {transCode}=yield select(({shRentIntentsRefundAuditInfo})=>shRentIntentsRefundAuditInfo);
      yield put({
    ...
    *getAuditRecodeData({payload},{select,call,put}){
      const {applyId,current,status}=yield select(({shRentIntentsRefundAuditInfo})=>shRentIntentsRefundAuditInfo);
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {todo} = yield select(({ shRentIntentsRefundAuditInfo }) => shRentIntentsRefundAuditInfo.promptObj);
      switch (todo) {
        case 'default':
