...
export default {
  namespace: 'secondHouseRentDealUploadIntentsAgreement',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/tradeManagement/secondHouseRentTrade/secondHouseRentDealUploadIntentsAgreement'){
          dispatch({
            type:'doInitState',
          })
  ...
  effects:{
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {todo} = yield select(({ secondHouseRentDealUploadIntentsAgreement }) => secondHouseRentDealUploadIntentsAgreement.promptObj);
      switch (todo) {
        case 'default':
    ...
    *doUpLoad({payload},{select,call,put}){
      const {upLoadPicList,transCode}=yield select(({secondHouseRentDealUploadIntentsAgreement})=>secondHouseRentDealUploadIntentsAgreement);
