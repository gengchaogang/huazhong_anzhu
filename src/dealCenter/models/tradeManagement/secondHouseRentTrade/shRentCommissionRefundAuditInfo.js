...
export default {
  namespace: 'shRentCommissionRefundAuditInfo',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/tradeManagement/secondHouseRentTrade/shRentCommissionRefundAuditInfo') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *readyRevokeApply({payload},{put,select}){
      const {transCode}=yield select(({shRentCommissionRefundAuditInfo})=>shRentCommissionRefundAuditInfo);
      yield put({
    ...
    *getAuditRecodeData({payload},{select,call,put}){
      const {applyId,current,status}=yield select(({shRentCommissionRefundAuditInfo})=>shRentCommissionRefundAuditInfo);
      const {data}=yield call(getCommissionRefundAuditRecord,{applyId,loadAllAutitRecord:true});
        ...
        if(data.status==='success'){
          ...
          const showData={
            orderInfo:{
              ...
              intentionRentAmount:renderRentMeony(orderinfo.totalPrice,orderinfo.resourceType),
              commissionRatio:isNullRate(orderinfo.commissionRate,'-'),
              commissionAmount:renderMoneyStr(orderinfo.amount,'-'),
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {todo} = yield select(({ shRentCommissionRefundAuditInfo }) => shRentCommissionRefundAuditInfo.promptObj);
      switch (todo) {
        case 'default':
