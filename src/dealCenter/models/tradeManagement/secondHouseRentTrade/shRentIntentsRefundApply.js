...
export default {
  namespace: 'shRentIntentsRefundApply',
  state:lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/tradeManagement/secondHouseRentTrade/shRentIntentsRefundApply') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *getIntentionRefundPreDataJSON({payload},{call,put,}){
      ...
      if(!!data){
        if(data.status==='success' && !!data.data){
          ...
          try {
            resourcesInfo = JSON.parse(result.intentions.resourcesInfo)
          } catch (e) {
            resourcesInfo = null
          }
          const orderInfo={
            ...
            // unitPrice:isNull(result.intentions.unitPrice,'-'),
            intentionRentAmount:renderRentMeony(result.intentions.actualRent,result.intentions.propertyType),
            intentionAmount:renderMoneyStr(result.intentions.intentionAmount,resourcesInfo.resourcesType),
    ...
    *postRefundApplyData({payload},{call,select,put}){
      ...
      const {upLoadPicList,transCode}=yield select(({shRentIntentsRefundApply})=>shRentIntentsRefundApply);
      const {data}=yield call(postIntentionRefundApplyFetch,{
        ...
        intentionReturnedToOwner:isNull(payload.refundTo,'租房者')==='业主',
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {todo} = yield select(({ shRentIntentsRefundApply }) => shRentIntentsRefundApply.promptObj);
      switch (todo) {
        case 'default':
