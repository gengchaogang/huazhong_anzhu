...
export default {
  namespace: 'shSellCommissionRefundAuditInfo',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/tradeManagement/secondHouseSellTrade/shSellCommissionRefundAuditInfo') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *readyRevokeApply({payload},{put,select}){
      const {transCode}=yield select(({shSellCommissionRefundAuditInfo})=>shSellCommissionRefundAuditInfo);
      yield put({
    ...
    *getAuditRecodeData({payload},{select,call,put}){
      const {applyId,current,status}=yield select(({shSellCommissionRefundAuditInfo})=>shSellCommissionRefundAuditInfo);
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {todo} = yield select(({ shSellCommissionRefundAuditInfo }) => shSellCommissionRefundAuditInfo.promptObj);
      switch (todo) {
        case 'default':
