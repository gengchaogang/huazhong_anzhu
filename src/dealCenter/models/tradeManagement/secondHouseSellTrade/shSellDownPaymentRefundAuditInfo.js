...
export default {
  namespace: 'shSellDownPaymentRefundAuditInfo',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/tradeManagement/secondHouseSellTrade/shSellDownPaymentRefundAuditInfo') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *readyRevokeApply({payload},{put,select}){
      const {transCode}=yield select(({shSellDownPaymentRefundAuditInfo})=>shSellDownPaymentRefundAuditInfo);
      yield put({
    ...
    *getAuditRecodeData({payload},{select,call,put}){
      const {applyId,current,status}=yield select(({shSellDownPaymentRefundAuditInfo})=>shSellDownPaymentRefundAuditInfo);
      const {data}=yield call(getPayDownRefundAuditRecord,{applyId,loadAllAutitRecord:true});
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {todo} = yield select(({ shSellDownPaymentRefundAuditInfo }) => shSellDownPaymentRefundAuditInfo.promptObj);
      switch (todo) {
        case 'default':
