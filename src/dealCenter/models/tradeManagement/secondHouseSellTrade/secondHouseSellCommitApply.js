...
}from '../../../services/tradeManagement/secondHouseSellTrade/secondHouseSellCommitApply'
const initState={
  transCode:null,
  trackJSON:null,
  loading:true,
  ...
  releaseIntention:null,//意向金释放
  releasePayDown:null,//首付款释放
  canBeRelease:false,//是否有资金释放
...
export default {
  namespace: 'secondHouseSellCommitApply',
  state: initState,
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/tradeManagement/secondHouseSellTrade/secondHouseSellCommitApply') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *getReportTransBasicInfo({payload},{call,put}){
      ...
      if(!!data){
        if(data.status==='success' && !!data.data){
          ...
          try{
            surplusCommissionAmount=floorTwoNumber(accMul(Number(data.data.commissionAmount),accSub(1,Number(data.data.platformCommissionRate))));
          }catch(e){
            yield put({
              type: 'switchPrompt',
              payload:{
          ...
          const houseInfo={
            ...
            price:renderUnitPriceStr(data.data.unitPrice),
            totalPrice:renderTotalMoneyStr(data.data.totalPrice,'-'),
            commitCommission:renderMoneyStr(data.data.commissionAmount),
          ...
          if(isNull(data.data.transactionMode,'')==='合作成交'){
            ...
            if(checkNum(data.data,'customerBrokerCommissionRate')){
              commissionType='rate';
              commitInfo.customerBrokerCommissionAmount=floorTwoNumber(accMul(Number(surplusCommissionAmount),Number(data.data.customerBrokerCommissionRate)));
              commitInfo.brokerCommissionAmount=floorTwoNumber(Number(surplusCommissionAmount)-commitInfo.customerBrokerCommissionAmount);
            }else{
              commissionType='quota';
              commitInfo.brokerCommissionAmount=floorTwoNumber(accSub(Number(surplusCommissionAmount),Number(isNull(data.data.customerBrokerCommissionAmount,0))));
    ...
    *checkPreCommitApply({payload},{select,put}){
      const {
        ...
        applyPreDataInfo,
        commissionType,
      }=yield select(({secondHouseSellCommitApply})=>secondHouseSellCommitApply);
      if(!!payload){
        const {
          ownerBankCard,
          auditor,
        ...
        let releaseinfo=[];
        if(!!payload.downPay_releaseType){
          releaseinfo.push({
            inComeSerialNumber:payload.downPay_inComeSerialNumber,
            releaseAmount:payload.downPay_releaseAmount,
            inComeOrderNumber:releasePayDown.inComeOrderNumber,
            releaseOwner:releasePayDown.releaseTo==='业主',
            releaseType:payload.downPay_releaseType,
          })
        }
        ...
        try{
          if(!!payload.brokerCommissionAmount){
            ...
            brokerCommissionAmount=floorTwoNumber(accMul(Number(payload.surplusCommissionAmount),getNumByPersent(payload.brokerCommissionRate)));
          }
          if(!!payload.customerBrokerCommissionAmount){
            ...
            customerBrokerCommissionAmount=floorTwoNumber(accMul(Number(payload.surplusCommissionAmount),getNumByPersent(payload.customerBrokerCommissionRate)));
            brokerCommissionAmount=floorTwoNumber(accMul(Number(payload.surplusCommissionAmount),accSub(1,getNumByPersent(payload.customerBrokerCommissionRate))));
          }
        }catch(e){
        ...
        const commitApplyObj={
          ...
          brokerCommissionAmount,
          customerBrokerCommissionRate:floorTwoNumber(getNumByPersent(isNull(payload.customerBrokerCommissionRate,''))),
          customerBrokerCommissionAmount,
          transCode,
        }
        console.log('applyPreDataInfo',applyPreDataInfo);
        ...
        if(applyPreData.transactionMode==='合作成交'){
          ...
          commissionArr.push({
            label:'经纪人佣金比例',
            value:isNullRate(applyPreData.brokerCommissionRate,'-'),
          })
          commissionArr.push({
            label:'经纪人佣金金额',
            value:`${isNull(commitApplyObj.surplusCommissionAmount,'-')}元`,
    ...
    *doCommitAppply({payload},{select,call,put}){
      const {transCode,auditorList,commitApplyInfo,isFailCommit,applyModal:{upLoadPicList,}}=yield select(({secondHouseSellCommitApply})=>secondHouseSellCommitApply);
      if(!!payload){
        const {
          auditor,
          failCommitReason,
        ...
        let auditorName;
        console.log('failCommitReason',failCommitReason);
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {todo} = yield select(({ secondHouseSellCommitApply }) => secondHouseSellCommitApply.promptObj);
      switch (todo) {
        case 'default':
  ...
  reducers: {
    ...
    //【资金释放】首付款释放目标变化
    onDowResToChange(state,action){
      return {...state,releasePayDown:{...state.releasePayDown,releaseTo:action.payload}}
    },
    //【资金释放】意向金释放目标变化
