...
}from '../../../services/tradeManagement/secondHouseSellTrade/secondHouseSellDealUploadDownPaymentAgreement'
...
export default {
  namespace: 'secondHouseSellDealUploadDownPaymentAgreement',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/tradeManagement/secondHouseSellTrade/secondHouseSellDealUploadDownPaymentAgreement'){
          dispatch({
            type:'doInitState',
          })
  ...
  effects:{
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {todo} = yield select(({ secondHouseSellDealUploadDownPaymentAgreement }) => secondHouseSellDealUploadDownPaymentAgreement.promptObj);
      switch (todo) {
        case 'default':
    ...
    *doUpLoad({payload},{select,call,put}){
      const {upLoadPicList,transCode}=yield select(({secondHouseSellDealUploadDownPaymentAgreement})=>secondHouseSellDealUploadDownPaymentAgreement);
