...
export default {
  namespace: 'shSellCommissionRefundApply',
  state:lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/tradeManagement/secondHouseSellTrade/shSellCommissionRefundApply') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
      ...
      if(!!data){
        if(data.status==='success' && !!data.data){
          ...
          const orderInfo={
            ...
            undertakerPhone:isNull(result.undertakerPhone,'-'),
            totalPrice:renderTotalMoneyStr(result.commission.totalPrice,'-'),
            commissionRatio:isNullRate(result.commission.commissionRate,'-'),
    ...
    *postRefundApplyData({payload},{call,select,put}){
      ...
      const {upLoadPicList,transCode}=yield select(({shSellCommissionRefundApply})=>shSellCommissionRefundApply);
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {todo} = yield select(({ shSellCommissionRefundApply }) => shSellCommissionRefundApply.promptObj);
      switch (todo) {
        case 'default':
