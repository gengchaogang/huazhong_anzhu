...
}from '../../../services/tradeManagement/secondHouseSellTrade/secondHouseSellDealUploadCommissionAgreement'
...
export default {
  namespace: 'secondHouseSellDealUploadCommissionAgreement',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/tradeManagement/secondHouseSellTrade/secondHouseSellDealUploadCommissionAgreement'){
          dispatch({
            type:'doInitState',
          })
  ...
  effects:{
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {todo} = yield select(({ secondHouseSellDealUploadCommissionAgreement }) => secondHouseSellDealUploadCommissionAgreement.promptObj);
      switch (todo) {
        case 'default':
    ...
    *doUpLoad({payload},{select,call,put}){
      const {upLoadPicList,transCode}=yield select(({secondHouseSellDealUploadCommissionAgreement})=>secondHouseSellDealUploadCommissionAgreement);
