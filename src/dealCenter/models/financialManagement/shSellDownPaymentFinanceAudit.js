...
import {
  shSellDownPaymentFinanceAuditPassFetch,
  shSellDownPaymentFinanceAuditRejectFetch,
...
export default {
  namespace: 'shSellDownPaymentFinanceAudit',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/financialManagement/secondHouseSellExamine/shSellDownPaymentFinanceAudit') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *doAudit({payload},{select,call,put}){
      ...
      const {todo}=payload;
      const {recordId}=yield select(({shSellDownPaymentFinanceAudit})=>shSellDownPaymentFinanceAudit);
      const {data}=yield call((todo==='pass'?shSellDownPaymentFinanceAuditPassFetch:shSellDownPaymentFinanceAuditRejectFetch),{...renderSHAduitRequestAgr(payload),recordId,});
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {promptObj:{todo,},recordId,applyId,} = yield select(({ shSellDownPaymentFinanceAudit }) => shSellDownPaymentFinanceAudit);
      switch (todo) {
        case 'default':
