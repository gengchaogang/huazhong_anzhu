...
export default {
  namespace: 'shSellCommissionFinanceAudit',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/financialManagement/secondHouseSellExamine/shSellCommissionFinanceAudit') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *doAudit({payload},{select,call,put}){
      ...
      const {todo}=payload;
      const {recordId}=yield select(({shSellCommissionFinanceAudit})=>shSellCommissionFinanceAudit);
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {promptObj:{todo,},recordId,applyId,} = yield select(({ shSellCommissionFinanceAudit }) => shSellCommissionFinanceAudit);
      switch (todo) {
        case 'default':
