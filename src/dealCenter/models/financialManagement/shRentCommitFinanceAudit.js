...
export default {
  namespace: 'shRentCommitFinanceAudit',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/financialManagement/secondHouseLeaseExamine/shRentCommitFinanceAudit') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *doAudit({payload},{select,call,put}){
      ...
      const {todo}=payload;
      const {recordId}=yield select(({shRentCommitFinanceAudit})=>shRentCommitFinanceAudit);
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {promptObj:{todo,},recordId,applyId,} = yield select(({ shRentCommitFinanceAudit }) => shRentCommitFinanceAudit);
      switch (todo) {
        case 'default':
