...
export default {
  namespace: 'shSellIntentsFinanceAudit',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/financialManagement/secondHouseSellExamine/shSellIntentsFinanceAudit') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *doAudit({payload},{select,call,put}){
      ...
      const {todo}=payload;
      const {recordId}=yield select(({shSellIntentsFinanceAudit})=>shSellIntentsFinanceAudit);
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {promptObj:{todo,},recordId,applyId,} = yield select(({ shSellIntentsFinanceAudit }) => shSellIntentsFinanceAudit);
      switch (todo) {
        case 'default':
