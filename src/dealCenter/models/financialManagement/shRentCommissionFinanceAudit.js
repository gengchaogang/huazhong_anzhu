...
import {
  shSellCommissionFinanceAuditPassFetch,
  shSellCommissionFinanceAuditRejectFetch,
...
export default {
  namespace: 'shRentCommissionFinanceAudit',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/financialManagement/secondHouseLeaseExamine/shRentCommissionFinanceAudit') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
        ...
        if(data.status==='success'){
          ...
          const showData={
            orderInfo:{
              ...
              intentionRentAmount:renderRentMeony(orderinfo.totalPrice,orderinfo.resourcesType),
            ...
            applyInfo:{
              formData:[
                {
                  label:'退款金额',
                  value:renderMoneyStr(applyinfo.refundCommissionAmount,''),
                },{
                  label:'退款原因',
    ...
    *doAudit({payload},{select,call,put}){
      ...
      const {todo}=payload;
      const {recordId}=yield select(({shRentCommissionFinanceAudit})=>shRentCommissionFinanceAudit);
      const {data}=yield call((todo==='pass'?shSellCommissionFinanceAuditPassFetch:shSellCommissionFinanceAuditRejectFetch),{...renderSHAduitRequestAgr(payload),recordId,});
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {promptObj:{todo,},recordId,applyId,} = yield select(({ shRentCommissionFinanceAudit }) => shRentCommissionFinanceAudit);
      switch (todo) {
        case 'default':
