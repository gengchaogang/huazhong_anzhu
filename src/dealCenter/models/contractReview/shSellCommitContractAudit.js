...
export default {
  namespace: 'shSellCommitContractAudit',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/contractReview/secondHandHouseSalesAudit/shSellCommitContractAudit') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *doAudit({payload},{select,call,put}){
      ...
      const {todo}=payload;
      const {recordId}=yield select(({shSellCommitContractAudit})=>shSellCommitContractAudit);
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {promptObj:{todo,},recordId,applyId,} = yield select(({ shSellCommitContractAudit }) => shSellCommitContractAudit);
      switch (todo) {
        case 'default':
