...
import {
  shSellDownPaymentContractAuditPassFetch,
  shSellDownPaymentContractAuditRejectFetch,
...
export default {
  namespace: 'shSellDownPaymentContractAudit',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/contractReview/secondHandHouseSalesAudit/shSellDownPaymentContractAudit') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *getAuditRecodeData({payload},{put,call}){
      const {data}=yield call(getPayDownRefundAuditRecord,{applyId:payload});
      if(!!data){
        if(data.status==='success'){
          ...
          const showData={
            orderInfo:{
              ...
              payDownRatio:isNullRate(orderinfo.firstpaymentRatio,'-'),
              deductedIntention:isNull(orderinfo.deductedIntention,'-'),
    ...
    *doAudit({payload},{select,call,put}){
      ...
      const {todo}=payload;
      const {recordId}=yield select(({shSellDownPaymentContractAudit})=>shSellDownPaymentContractAudit);
      const {data}=yield call((todo==='pass'?shSellDownPaymentContractAuditPassFetch:shSellDownPaymentContractAuditRejectFetch),{...renderSHAduitRequestAgr(payload),recordId,});
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {promptObj:{todo,},recordId,applyId,} = yield select(({ shSellDownPaymentContractAudit }) => shSellDownPaymentContractAudit);
      switch (todo) {
        case 'default':
