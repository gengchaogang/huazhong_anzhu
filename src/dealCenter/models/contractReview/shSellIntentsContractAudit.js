...
export default {
  namespace: 'shSellIntentsContractAudit',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/contractReview/secondHandHouseSalesAudit/shSellIntentsContractAudit') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *doAudit({payload},{select,call,put}){
      ...
      const {todo}=payload;
      const {recordId}=yield select(({shSellIntentsContractAudit})=>shSellIntentsContractAudit);
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {promptObj:{todo,},recordId,applyId,} = yield select(({ shSellIntentsContractAudit }) => shSellIntentsContractAudit);
      switch (todo) {
        case 'default':
