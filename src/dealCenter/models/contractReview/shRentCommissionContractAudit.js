...
export default {
  namespace: 'shRentCommissionContractAudit',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/contractReview/secondHandHouseRentalAudit/shRentCommissionContractAudit') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *doAudit({payload},{select,call,put}){
      ...
      const {todo}=payload;
      const {recordId}=yield select(({shRentCommissionContractAudit})=>shRentCommissionContractAudit);
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {promptObj:{todo,},recordId,applyId,} = yield select(({ shRentCommissionContractAudit }) => shRentCommissionContractAudit);
      switch (todo) {
        case 'default':
