...
import {
  shSellCommitContractAuditPassFetch,
  shSellCommitContractAuditRejectFetch,
...
export default {
  namespace: 'shRentCommitContractAudit',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/contractReview/secondHandHouseRentalAudit/shRentCommitContractAudit') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
    ...
    *getAuditRecodeData({payload},{put,call}){
      ...
      if(!!data){
        if(data.status==='success'){
          ...
          //判断审核状态
          let canAudit = true;
          const auditStatus=renderAuditStatus('commit',isNull(applyinfo.auditStatus,''),'contract');
          const stepList=[
            {
              title:'申请成交'
            },{
    ...
    *doAudit({payload},{select,call,put}){
      ...
      const {todo}=payload;
      const {recordId}=yield select(({shRentCommitContractAudit})=>shRentCommitContractAudit);
      const {data}=yield call((todo==='pass'?shSellCommitContractAuditPassFetch:shSellCommitContractAuditRejectFetch),{...renderSHAduitRequestAgr(payload),recordId,});
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {promptObj:{todo,},recordId,applyId,} = yield select(({ shRentCommitContractAudit }) => shRentCommitContractAudit);
      switch (todo) {
        case 'default':
