...
import {
  shSellIntentsContractAuditPassFetch,
  shSellIntentsContractAuditRejectFetch,
...
export default {
  namespace: 'shRentIntentsContractAudit',
  state: lodash.cloneDeep(initState),
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname ==='/contractReview/secondHandHouseRentalAudit/shRentIntentsContractAudit') {
          dispatch({
            type:'initComponent',
  ...
  effects:{
        ...
        if(data.status==='success'){
          ...
          const showData={
            orderInfo:{
              ...
              intentionRentAmount:renderMoneyStr(orderinfo.actualRent,'-'),
    ...
    *doAudit({payload},{select,call,put}){
      ...
      const {todo}=payload;
      const {recordId}=yield select(({shRentIntentsContractAudit})=>shRentIntentsContractAudit);
      const {data}=yield call((todo==='pass'?shSellIntentsContractAuditPassFetch:shSellIntentsContractAuditRejectFetch),{...renderSHAduitRequestAgr(payload),recordId,});
    ...
    *closePrompt({ payload }, { select,call, put }){
      const {promptObj:{todo,},recordId,applyId,} = yield select(({ shRentIntentsContractAudit }) => shRentIntentsContractAudit);
      switch (todo) {
        case 'default':
